...
class Config:
    ...
    def set_config(self, jsonpath, value, print=True):
        ...
        if (
            ...
            and (shared_dir_path := pathlib.Path(shared_dir))
    ...
    def apply_preset_from_pr_args(self):
        for config_key in self.get_config("$", print=False).keys():
            if not config_key.startswith(PR_ARG_KEY):
                continue
            if config_key == f"{PR_ARG_KEY}0":
                continue
    ...
    def detect_apply_light_profile(self, profile, name_suffix="light"):
        if os.environ.get("OPENSHIFT_CI"):
            ...
            logging.info(
                f"Running a '{name_suffix}' test ({job_name_safe}), applying the '{profile}' profile"
            )
    ...
    def detect_apply_metal_profile(self, profile):
        platform_type_cmd = run.run(
            "oc get infrastructure/cluster -ojsonpath={.status.platformStatus.type}",
            capture_stdout=True,
            capture_stderr=True,
            check=False,
        )
        ...
        logging.info(
            f"detect_apply_metal_profile: infrastructure/cluster.status.platformStatus.type = {platform_type}"
        )
        if platform_type not in ("BareMetal", "None", "IBMCloud"):
            logging.info(
                "detect_apply_metal_profile: Assuming not running in a bare-metal environment."
            )
            return False
        logging.info(
            f"detect_apply_metal_profile: Assuming running in a bare-metal environment. Applying the '{profile}' profile."
        )
...
def _set_config_environ(testing_dir):
    ...
    if env_config_file_src := os.environ.get("TOPSAIL_FROM_CONFIG_FILE"):
        ...
        logging.info(
            f"Loading the configuration from TOPSAIL_FROM_CONFIG_FILE={config_file_src} ..."
        )
        ...
        logging.info(
            f"Reloading the config file from TOPSAIL project directory {config_file_src} ..."
        )
...
def get_command_arg(group, command, arg, prefix=None, suffix=None, mute=False):
    try:
        ...
        proc = run.run_toolbox_from_config(
            group,
            command,
            show_args=arg,
            prefix=prefix,
...
def init(testing_dir, apply_preset_from_pr_args=False, apply_config_overrides=True):
    ...
    if os.environ.get("TOPSAIL_LOCAL_CI_MULTI") == "true":
        logging.info(
            "config.init: running in a local-ci multi Pod, skipping apply_config_overrides and apply_preset_from_pr_args."
        )
        return

    if not apply_config_overrides:
        logging.info(
            "config.init: running with 'apply_config_overrides', skipping the overrides. Saving it as 'overrides' field in the project configuration."
        )
    ...
    variable_overrides_to_apply = project.get_config(
        "ci_presets.variable_overrides", {}, warn=False
    )
