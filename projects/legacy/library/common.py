...
def cleanup_cluster():
    ...
    has_busy_cluster_ns = run.run(
        f"oc get ns -oname -l{busy_cluster_ns_label_key}={busy_cluster_ns_label_value}",
        capture_stdout=True,
    ).stdout
    ...
    logging.info(
        f"Detected {len(has_busy_cluster_ns.split())} busy-cluster namespaces. Running the cleanup."
    )
