...
def find_execution_logs(base_artifact_dir: Path) -> list[Path]:
    """
    Find all execution log files (_ansible.log, task.log) in the artifact directory
    ...
    for log_file in base_artifact_dir.rglob("_ansible.log"):
        if log_file.exists():
...
def read_failure_and_log(failure_file: Path) -> dict:
    """
    Read a FAILURE file and its corresponding log file (_ansible.log or task.log)
    ...
    # Check for different log file types in order of preference
    execution_log = failure_dir / "_ansible.log"
    ...
    if log_file and log_file.exists():
        try:
            with open(log_file) as f:
                ...
                if log_type == "execution":
                    # Process execution logs - trim from beginning until after first "| "
                    lines = content.splitlines()
                    trimmed_lines = []
                    for line in lines:
                        # Trim line from beginning until after first "| "
                        if "| " in line:
        ...
        logger.warning(f"No log file found in {failure_dir} (checked: _ansible.log, task.log)")
        result["log_content"] = "No log file (_ansible.log or task.log) found in the same directory"
