...
def get_tests_timestamp_plots(entries, y_max, variables):
    ...
    tz_offset = datetime.datetime.fromtimestamp(
        now.timestamp()
    ) - datetime.datetime.utcfromtimestamp(now.timestamp())

    for entry in entries:
        if "tests_timestamp" not in entry.results.__dict__:
            continue

        for test_timestamp in entry.results.tests_timestamp:
            # convert to local timezone and remove timezone info
            start = test_timestamp.start + tz_offset
            start.replace(tzinfo=None)
            end = test_timestamp.end + tz_offset
            end.replace(tzinfo=None)

            settings_text_lst = [
                str(v) if k.startswith("*") else f"{k}={v}"
                for k, v in test_timestamp.settings.items()
                if k in variables
            ]
...
class Plot:
    ...
    def do_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        for entry in common.Matrix.all_records(settings, setting_lists):
            ...
            sort_index = (
                entry.get_settings()[ordered_vars[0]] if len(variables) == 1 else entry_name
            )

            for _metric in self.metrics:
                metric_name, metric_query = (
                    list(_metric.items())[0] if isinstance(_metric, dict) else (_metric, _metric)
                )
        ...
        if show_test_timestamps:
            tests_timestamp_y_position, plots = get_tests_timestamp_plots(
                common.Matrix.all_records(settings, setting_lists), y_max, variables
            )
            data += plots
        ...
        if as_timeline:
            ...
            fig.update_layout(
                ...
                yaxis=dict(
                    title=self.y_title,
                    range=[tests_timestamp_y_position if show_test_timestamps else 0, y_max * 1.05],
                ),
            ...
            fig.update_layout(title=plot_title, title_x=0.5, yaxis=dict(title=self.y_title))
