    ...
    def do_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        for entry in common.Matrix.all_records(settings, setting_lists):
            ...
            for _metric in self.metrics:
                ...
                for metric in self.filter_metrics(entry, self.get_metrics(entry, metric_name)):
                    ...
                    if metric.metric.get("container") == "POD":
                        continue
                    ...
                    if "_sum_" in metric_name:
                        ...
                        legend_group = (
                            metric.metric.get("pod", "<no podname>")
                            + "/"
                            + metric.metric.get("container", self.container_name)
        ...
        if as_timeline:
            ...
            fig.update_layout(
                ...
                title_x=0.5,
                yaxis=dict(
                    title=self.y_title + (" (in Gi)" if self.is_memory else ""),
