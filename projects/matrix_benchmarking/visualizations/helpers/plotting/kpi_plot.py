...
def generateData(entries, x_key, kpi_name, _variables, filter_key=None, filter_value=None):
    ...
    for entry in entries:
        ...
        datum = dict()
        datum[x_key] = entry.settings.__dict__[x_key] if x_key else None
...
class KpiPlot:
    ...
    def do_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        data, properties = generateData(
            entries, x_key, cfg__kpi_name, variables, cfg__filter_key, cfg__filter_value
        )
        ...
        if is_list_kpi:
            title += " (list KPI)"
        fig.update_yaxes(
            title=("❮ " if y_lower_better else "") + y_title + (" ❯" if not y_lower_better else "")
        )
        fig.update_layout(
            title=title,
            title_x=0.5,
    ...
    def do_plot(self, *args):
        ...
        for kpi_name in first_entry.results.lts.kpis.keys():
            ...
            for _ in range(len(ordered_vars)):
                ...
                header += report.Plot_and_Text("KPI Plot", kpi_args)
