...
class KPITableReport:
    ...
    def do_plot(self, *args):
        ...
        kpi_names = (
            set(
                list(all_kpi_results_data[0].keys())[
                    max([metadata_column_count, len(variable_labels) + 1]) :
                ]
            )
            if all_kpi_results_data
            else set()
        )
        ...
        if self.settings_version:
            ...
            header.append(
                analyze_report._generate_results_overview(
                    all_test_results_data, ordered_vars, kpi_names, kpis_common_prefix, warn=False
                )
            )
        ...
        if self.labels_version:
            ...
            header.append(
                analyze_report._generate_configuration_overview(all_kpi_settings, variable_labels)
            )
            ...
            header.append(
                analyze_report._generate_results_overview(
                    all_kpi_results_data, variable_labels, kpi_names, kpis_common_prefix
                )
            )
