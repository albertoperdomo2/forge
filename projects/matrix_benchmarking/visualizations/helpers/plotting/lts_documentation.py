...
class LtsDocumentationReport:
    ...
    def do_plot(self, *args):
        ...
        for entry in common.Matrix.all_records(settings, setting_lists):
            header += [
                html.H1(entry.get_name(variables or [s for s in settings.keys() if s != "stats"]))
            ]
    ...
    metadata += [
        html.Li(
            [
                html.B("urls:"),
                html.Ul(
                    [html.Li(html.A(k, href=v)) for k, v in lts.metadata.settings.urls.items()]
                ),
            ]
        )
    ]
        ...
        html.Li(
            [
                ...
                html.Ul(
                    [
                        html.Li(f"{k}: {v}")
                        for k, v in lts.metadata.settings.__dict__.items()
                        if k not in common.LTS_META_KEYS
                    ]
                ),
            ]
        )
                ...
                [
                    html.P(
                        [
                            html.B(name),
                            "⮕",
                            html.Code(f"{kpi.value} {kpi.unit}"),
                            html.Br(),
                            html.I(kpi.help),
                            html.Br(),
                        ]
