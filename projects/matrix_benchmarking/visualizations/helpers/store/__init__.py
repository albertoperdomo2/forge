...
class BaseStore:
    def __init__(
        ...
        *,
        ...
        parse_always,
        parse_once,
        generate_lts_payload=None,
        lts_payload_model=None,
        models_kpis=None,
        get_kpi_labels=None,
    ):
    ...
    def register_important_file(self, base_dirname, filename):
        ...
        if pathlib.Path(filename).is_absolute():
            logging.warning(
                f"File '{filename}' is an absolute path. Should be relative to {base_dirname}."
            )
    ...
    def resolve_artifact_dirnames(self, dirname):
        ...
        for artifact_dirname, unresolved_dirname in self.artifact_dirnames.__dict__.items():
            ...
            if direct_resolution.exists():
                ...
                logging.warning(
                    f"Cannot resolve {artifact_dirname} glob '{unresolved_dirname}' in '{dirname}'"
                )
            else:
                if len(resolutions) > 1:
                    logging.debug(
                        f"Found multiple resolutions for {artifact_dirname} glob '{unresolved_dirname}' in '{dirname}': {resolutions}. Taking the last one"
                    )
    ...
    def parse_directory(self, fn_add_to_matrix, dirname, import_settings, exit_code):
        ignore_cache = os.environ.get("MATBENCH_STORE_IGNORE_CACHE", False) in (
            "yes",
            "y",
            "true",
            "True",
    ...
    def build_lts_payloads(self):
        if not self.lts_payload_model:
            raise ValueError(
                "Cannot build the LTS payload: no `lts_payload_model` defined in this workload."
            )
        ...
        for entry in common.Matrix.processed_map.values():
            ...
            validate_lts_payload(
                self.lts_payload_model, lts_payload, entry.import_settings, reraise=True
            )
...
def validate_lts_payload(lts_payload_model, payload, import_settings, reraise=False):
    try:
        json_lts = json.dumps(
            payload, indent=4, default=functools.partial(json_dumper, strict=False)
        )
    ...
    try:
        ...
        log_fct("lts-error: Failed to validate the generated LTS payload against the model")
