...
def _get_container_cpu_queries(cluster_role, labels):
    ...
    return [
        {
            f"{cluster_role}__container_cpu__{metric_name}": "rate(container_cpu_usage_seconds_total{"
            + labels_str
            + "}[5m])"
        },
        {
            f"{cluster_role}__container_sum_cpu__{metric_name}": "sum(rate(container_cpu_usage_seconds_total{"
            + labels_str
            + "}[5m]))"
        },
        {
            f"{cluster_role}__container_cpu_requests__{metric_name}": "kube_pod_container_resource_requests{"
            + labels_str
            + ",resource='cpu'}"
        },
        {
            f"{cluster_role}__container_cpu_limits__{metric_name}": "kube_pod_container_resource_limits{"
            + labels_str
            + ",resource='cpu'}"
        },
    ]
...
def _get_container_mem_queries(cluster_role, labels):
    ...
    return [
        {
            f"{cluster_role}__container_memory_working_set_bytes__{metric_name}": "container_memory_working_set_bytes{"
            + labels_str
            + "}"
        },
        {
            f"{cluster_role}__container_memory_usage_bytes__{metric_name}": "container_memory_usage_bytes{"
            + labels_str
            + "}"
        },
        {
            f"{cluster_role}__container_memory_rss__{metric_name}": "container_memory_rss{"
            + labels_str
            + "}"
        },
        {
            f"{cluster_role}__container_memory_requests__{metric_name}": "kube_pod_container_resource_requests{"
            + labels_str
            + ",resource='memory'}"
        },
        {
            f"{cluster_role}__container_memory_limits__{metric_name}": "kube_pod_container_resource_limits{"
            + labels_str
            + ",resource='memory'}"
        },
        {
            f"{cluster_role}__container_max_memory__{metric_name}": "container_memory_max_usage_bytes{"
            + labels_str
            + "}"
        },
    ]
...
def _get_cluster_mem_queries(cluster_role):
    return [
        ...
        {
            f"{cluster_role}__cluster_memory_usage": """   sum(
...
"""
        ...
        {
            f"{cluster_role}__cluster_memory_requests": """   sum(
...
def _get_cluster_cpu_queries(cluster_role):
    return [
        {
            f"{cluster_role}__cluster_cpu_requests": """   sum(
...
"""
        ...
        {
            f"{cluster_role}__cluster_cpu_usage": """   sum(
        (
...
def _get_pod_disk_rate_queries(cluster_role, labels):
    ...
    return [
        {
            f"{cluster_role}__container_fs_reads_bytes_irate{metric_name}": "(sum(irate(container_fs_reads_bytes_total{"
            + labels_str
            + "}[5m])) by (pod, namespace))"
        },
        {
            f"{cluster_role}__container_fs_writes_bytes_irate{metric_name}": "(sum(irate(container_fs_writes_bytes_total{"
            + labels_str
            + "}[5m])) by (pod, namespace))"
        },
    ]
...
def _get_pod_disk_total_queries(cluster_role, labels):
    ...
    return [
        {
            f"{cluster_role}__container_fs_reads_bytes_total{metric_name}": "(sum(container_fs_reads_bytes_total{"
            + labels_str
            + "})) by (pod, namespace))"
        },
        {
            f"{cluster_role}__container_fs_writes_bytes_total{metric_name}": "(sum(container_fs_writes_bytes_total{"
            + labels_str
            + "}) by (pod, namespace))"
        },
    ]
...
def _get_pod_network_rate_queries(cluster_role, labels):
    ...
    return [
        {
            f"{cluster_role}__container_network_receive_bytes_irate__{metric_name}": "(sum(irate(container_network_receive_bytes_total{"
            + labels_str
            + "}[5m])) by (pod, namespace, interface)) + on(namespace,pod,interface) group_left(network_name) (pod_network_name_info)"
        },
        {
            f"{cluster_role}__container_network_transmit_bytes_irate__{metric_name}": "(sum(irate(container_network_transmit_bytes_total{"
            + labels_str
            + "}[5m])) by (pod, namespace, interface)) + on(namespace,pod,interface) group_left(network_name) (pod_network_name_info)"
        },
    ]
...
def _get_pod_network_total_queries(cluster_role, labels):
    ...
    return [
        {
            f"{cluster_role}__container_network_receive_bytes_total__{metric_name}": "(sum(container_network_receive_bytes_total{"
            + labels_str
            + "}) by (pod, namespace, interface)) + on(namespace,pod,interface) group_left(network_name) (pod_network_name_info)"
        },
        {
            f"{cluster_role}__container_network_transmit_bytes_total__{metric_name}": "(sum(container_network_transmit_bytes_total{"
            + labels_str
            + "}) by (pod, namespace, interface)) + on(namespace,pod,interface) group_left(network_name) (pod_network_name_info)"
        },
    ]
...
def _get_cluster_mem_cpu(cluster_role, register):
    ...
    if register:
        plotting_prom_cpu_memory.Plot(
            cluster_mem,
            f"{cluster_role} cluster memory usage",
        ...
        plotting_prom_cpu_memory.Plot(
            cluster_cpu,
            f"{cluster_role} cluster CPU usage",
            get_metrics=get_metrics(cluster_role),
            as_timestamp=True,
            is_cluster=True,
        )
...
def _get_disk_usage_metrics(cluster_role, register, label_sets, disk_metrics_names):
    ...
    for plot_name_labels in label_sets:
        plot_name, labels = list(plot_name_labels.items())[0]
        if plot_name not in disk_metrics_names:
            continue
        ...
        plotting_prom.Plot(
            disk_queries,
            f"Prom: {plot_name}: Disk total usage",
            get_metrics=get_metrics(cluster_role),
            as_timestamp=True,
    ...
    for plot_name_labels in label_sets:
        ...
        plotting_prom.Plot(
            disk_queries,
            f"Prom: {plot_name}: Disk rate usage",
            get_metrics=get_metrics(cluster_role),
            as_timestamp=True,
            title="Disk rate usage",
            y_title="in MB/s",
            y_divisor=1000 * 1000,
        )
...
def _get_network_usage_metrics(cluster_role, register, label_sets, network_metrics_names):
    ...
    for plot_name_labels in label_sets:
        plot_name, labels = list(plot_name_labels.items())[0]
        if plot_name not in network_metrics_names:
            continue
        ...
        plotting_prom.Plot(
            network_queries,
            f"Prom: {plot_name}: Network total usage",
            get_metrics=get_metrics(cluster_role),
            as_timestamp=True,
            title="Network total usage",
            y_title="in GB",
            y_divisor=1000 * 1000 * 1000,
            substract_first=True,
        )
    ...
    for plot_name_labels in label_sets:
        ...
        plotting_prom.Plot(
            network_queries,
            f"Prom: {plot_name}: Network rate usage",
            get_metrics=get_metrics(cluster_role),
            as_timestamp=True,
            title="Network rate usage",
            y_title="in MBps",
            y_divisor=1000 * 1000,
        )
...
def _get_control_plane_nodes_cpu_usage(cluster_role, register):
    ...
    def only_control_plane_nodes(entry, metrics):
        control_plane_nodes = [
            node.name
            for node in (
                entry.results.cluster_info.control_plane
                if getattr(entry.results, "cluster_info", [])
                else []
            )
        ]
...
def _get_control_plane_nodes(cluster_role, register):
    all_metrics = []
    all_metrics += _get_container_mem_cpu_metrics(
        cluster_role,
        register,
        [
            {
                f"{cluster_role.title()} ApiServer": dict(
                    namespace="openshift-kube-apiserver",
                    pod=["!~kube-apiserver-guard.*", "kube-apiserver-.*"],
                )
            }
        ],
    )
    all_metrics += _get_container_mem_cpu_metrics(
        ...
        [
            {
                f"{cluster_role.title()} ETCD": dict(
                    namespace="openshift-etcd", pod=["!~etcd-guard-.*", "etcd-.*"]
                )
            }
        ],
    )
...
def _get_apiserver_errcodes(cluster_role, register):
    ...
    apiserver_request_duration_metrics = [
        {
            f"{cluster_role.title()} API Server {verb} Requests duration": f'histogram_quantile(0.99, sum(rate(apiserver_request_duration_seconds_bucket{{apiserver="kube-apiserver", verb="{verb}", subresource!="log"}}[2m])) by (resource,subresource,le)) > 0'
        }
...
def get_cluster_metrics(
    ...
    *,
    ...
    if disk_metrics_names:
        all_metrics += _get_disk_usage_metrics(
            cluster_role, register, container_labels, disk_metrics_names
        )

    if network_metrics_names:
        all_metrics += _get_network_usage_metrics(
            cluster_role, register, container_labels, network_metrics_names
        )
