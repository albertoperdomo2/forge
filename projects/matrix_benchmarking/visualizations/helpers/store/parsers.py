...
def parse_env(dirname, test_config, capture_state_dir):
    ...
    if dirname.name == "from_url":
        ...
        _prefix, _, from_env.test.test_path = source_url.partition(
            f"{from_env.test.run_id}/artifacts/"
        )
        pass
    else:
        main_artifact_dir = pathlib.Path(ansible_env["TOPSAIL_FROM_CONFIG_FILE"]).parent
        current_artifact_dir = pathlib.Path(ansible_env["ARTIFACT_DIR"])
        from_env.test.test_path = current_artifact_dir.relative_to(main_artifact_dir)
    ...
    if ansible_env.get("OPENSHIFT_CI") == "truez":
        from_env.test.ci_engine = "OPENSHIFT_CI"
        job_spec = json.loads(ansible_env["JOB_SPEC"].strip("'"))
    ...
    if test_config and test_config.get("export_artifacts.enabled", False):
...
def parse_nodes_info(dirname, capture_state_dir, sutest_cluster=True):
    ...
    for node in nodeList["items"]:
        ...
        node_info.instance_type = node["metadata"]["labels"].get(
            "node.kubernetes.io/instance-type", "N/A"
        )

        node_info.control_plane = (
            "node-role.kubernetes.io/control-plane" in node["metadata"]["labels"]
            or "node-role.kubernetes.io/master" in node["metadata"]["labels"]
        )
        ...
        if node["metadata"]["labels"].get("nvidia.com/gpu.present"):
            ...
            if node["metadata"]["labels"].get("nvidia.com/gpu.memory"):
                node_info.gpu.memory = (
                    int(node["metadata"]["labels"].get("nvidia.com/gpu.memory")) / 1000
                )
        ...
        node_info.allocatable.memory = float(
            k8s_quantity.parse_quantity(node["status"]["allocatable"]["memory"])
        )
        node_info.allocatable.memory = float(
            k8s_quantity.parse_quantity(node["status"]["allocatable"]["memory"])
        )
        node_info.allocatable.cpu = float(
            k8s_quantity.parse_quantity(node["status"]["allocatable"]["cpu"])
        )
...
def extract_cluster_info(nodes_info):
    ...
    cluster_info.node_count = [
        node_info for node_info in nodes_info.values() if node_info.sutest_cluster
    ]

    cluster_info.control_plane = [
        node_info
...
def parse_rhods_info(dirname, capture_state_dir, version_name=None):
    ...
    if capture_state_dir is None:
        logging.error(
            "parse_rhods_info: `capture_state_dir` not available, returning dummy values :/"
        )
    ...
    if version_name:
        rhods_info.full_version = (
            f"{rhods_info.version}-{version_name}+{rhods_info.createdAt.strftime('%Y-%m-%d')}"
        )
    else:
...
def parse_local_env(dirname):
    ...
    try:
        with open(dirname / "source_url") as f:  # not an important file
            ...
            from_local_env.artifacts_basedir = pathlib.Path(
                urllib.parse.urlparse(from_local_env.source_url).path
            )
    except FileNotFoundError:
        pass

    logging.debug(
        f"Source_url: {from_local_env.source_url}, artifacts_basedir: {from_local_env.artifacts_basedir}"
    )
    ...
    if job_name.endswith("-plot"):
        ...
        from_local_env.source_url = pathlib.Path(
            os.path.relpath(dirname, pathlib.Path(os.environ["ARTIFACT_DIR"]))
        )
