...
def main(
    ...
    def run():
        ...
        client = (
            download_lts.connect_opensearch_client(kwargs) if not kwargs.get("dry_run") else None
        )
    ...
    index_body = {
        "mappings": {
            "properties": {
                ...
                "kpis": {"type": "flat_object"},
                "results": {"type": "flat_object"},
                "regression_results": {"type": "flat_object"},
            },
        },
        "settings": {
    ...
    if upload_by_kpi:
        for lts_payload, _, _ in lts_payloads:
            payload_json = json.dumps(
                lts_payload, default=functools.partial(parse.json_dumper, strict=False)
            )
...
def upload(client, workload_store, dry_run, opensearch_index, upload_by_kpi):
    ...
    for idx, (payload, start, end) in enumerate(lts_payloads):
        try:
            ...
            key = ",".join(
                f"{k}={v}" for k, v in settings_dict.items() if (not variables or k in variables)
            )
        except Exception as e:
        ...
        payload_json = json.dumps(
            payload, default=functools.partial(parse.json_dumper, strict=False)
        )
        payload_dict = json.loads(payload_json)
...
def upload_lts_to_opensearch(client, payload_dict, dry_run, opensearch_index):
    ...
    return upload_to_opensearch(
        client, payload_dict, payload_dict["metadata"]["test_uuid"], dry_run, opensearch_index
    )


def upload_kpis_to_opensearch(client, payload_dict, dry_run, opensearch_index):
    if "kpis" not in payload_dict.keys():
        logging.info("==> no KPI found in the payload.")
        return
...
def upload_regression_results_to_opensearch(client, payload_dict, dry_run, opensearch_index):
    if not payload_dict.get("regression_results"):
        logging.info("==> no regression results found in the payload.")
        return

    logging.info("Uploading regression results to opensearch ... (stub)")
    for idx, regression_result in enumerate(payload_dict["regression_results"]):
        ...
        upload_to_opensearch(
            client,
            regression_result,
            payload_dict["metadata"]["test_uuid"],
            True,
            regression_result_index,
        )


def upload_to_opensearch(client, document, document_id, dry_run, index):
    if dry_run:
        logging.info("==> skip upload (dry run)")
        return
