...
def get_benchmark_yaml_file(benchmark_file):
    ...
    if not benchmark_file:
        raise ValueError(
            "benchmark_file must be passed in the command-line or with the MATBENCH_BENCHMARK_FILE environment variable."
        )
            ...
            if env_file.suffix in (".yaml", ".json"):
                ...
                for k, v in doc.items():
                    if not isinstance(k, str):
                        logging.warning(
                            f"Env file {env_file} contains a key {k} that isn't a string ({k.__class__.__name__})"
                        )
                        continue
...
def update_kwargs_with_env(kwargs):
    ...
    for flag, current_value in kwargs.items():
        ...
        if not env_value:
            ...
            if not env_value:
                continue  # not set in an environment file, ignore
...
def check_mandatory_kwargs(kwargs, mandatory_flags, sensitive_flags=[]):
    ...
    for k, v in sorted(kwargs.items()):
        if v not in mandatory_flags and not v:
            continue
    ...
    for flag in mandatory_flags:
        ...
        logging.fatal(
            f"--{flag} must be set in CLI, in the benchmark file or though MATBENCH_{flag.upper()} environment variable."
        )
        err = True
