...
def register_all():
    for stat in TableStats.all_stats:
        ...
        if not isinstance(stat, TableStats):
            continue
...
class TableStats:
        ...
        class FutureValue:
            ...
            def value(myself):
                ...
                try:
                    ...
                    logging.error(
                        f"Failed to process field '{self.field}' with"
    ...
    def process_gathered_value_dev(self, entry):
        values = [self.process_value_dev(gathered_entry)[0] for gathered_entry in entry.results]
    ...
    def process_value_dev(self, entry):
        ...
        if isinstance(value, list):
            logging.warning(
                f"{entry.location}.results.{self.field} is a list of length {len(value)}"
            )
            value = value[0]
    ...
    def process_mean_std(self, entry):
        ...
        if not isinstance(value, list):
            logging.warning(
                f"{entry.location}.results.{self.field} is NOT a list of length ({values})"
            )
            values = [values]

        if not values:
            logging.warning("{entry.location}.results.{self.field} is an empty list")
            return 0, 0
    ...
    def do_hover(self, meta_value, variables, figure, data, click_info):
        ...
        try:
            xaxis_name = figure["layout"][xaxis]["title"]["text"]
        except KeyError:
            xaxis_name = ""
        ...
        graph = self.entry_to_hovergraph(entry) if entry else ""
    ...
    def entry_to_hovergraph(self, entry):
        ...
        for table_def, (table_name, table_rows) in entry.tables.items():
            if table_name != self.table and not (
                self.table.startswith("?.") and table_name.endswith(self.table[1:])
            ):
                continue
            ...
            try:
                ...
                try:
                    ...
                    logging.warning(
                        f"props_to_hovergraph: Cannot find {table_simple_name}.msg_ts or time field ..."
                    )
                    return ""
    ...
    def props_to_hoverlink(variables, props, value):
        for prop in props:
            try:
                ...
                continue  # not enough values to unpack (expected 2, got 1)
            variables[k] = v

        entry = common.Matrix.get_record(variables)
        if not entry:
            return None, "Error: record not found in matrix ..."
    ...
    def do_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        *second_vars, legend_var = ordered_vars
        ...
        if second_vars:
            ...
            for i, subplots_key in enumerate(
                sorted(variables[subplots_var], key=plotting.natural_keys)
            ):
                subplots[subplots_key] = f"x{i + 1}"
                ax = f"xaxis{i + 1}"
                layout[ax] = dict(
                    title=f"{subplots_var}<br>{subplots_key}",
                    type="category",
            ...
            layout["xaxis1"] = dict(
                type="category", showticklabels=True, tickfont=dict(size=18), tickangle=45
            )

        x = defaultdict(list)
        y = defaultdict(list)
        y_err = defaultdict(list)
        legend_keys = set()
        legend_names = set()
        legends_visible = []
        subplots_used = set()
        ...
        for entry in common.Matrix.all_records(settings, setting_lists):
            ...
            x_key = ", ".join(
                [f"{v}={settings[v]}" for v in reversed(second_vars) if v != subplots_var]
            )
        ...
        def prepare_scatter_short_err(legend_key):
            y_err_above = []
            y_err_below = []
            for _y, _y_error in zip(y[legend_key], y_err[legend_key], strict=False):
                # above == below iff len(_y_error) == 1
                if None in (_y, _y_error):
                    continue
        ...
        def prepare_scatter_long_err(legend_key):
            ...
            for _x, _y, _y_error in zip(
                ...
                x_err_data += (
                    x_err_current + list(reversed(x_err_current)) + [x_err_current[0], None]
                )

                y_err_data += y_err_above + list(reversed(y_err_below)) + [y_err_above[0], None]
                x_err_current = []
                y_err_above = []
                y_err_below = []
        ...
        for legend_key in legend_keys:
            ...
            if var_length >= 5 and DO_LOCAL_SORT:
                ...
                x[legend_key] = [
                    _x
                    for _y, _x in sorted(
                        zip(y[legend_key], x[legend_key], strict=False),
            ...
            if showlegend:
                legends_visible.append(legend_name)
        ...
        if do_sort and var_length <= 2:
            ...
            for _y, _x in sorted(zip(y.values(), x.keys(), strict=False)):
                name = _x[0]
                trace = get(name)
                data.remove(trace)
                data.append(trace)
