...
def get_permalink(args, full=False):
    settings = dict(zip(Matrix.settings.keys(), args[: len(Matrix.settings)], strict=False))

    def val(k, v):
        if isinstance(v, list):
            return "&".join(f"{k}={vv}" for vv in v)
        else:
            return f"{k}={v}"
    ...
    *_, custom_cfg, custom_cfg_saved, settings_order, custom_cfg_saved_state = args
    ...
    if custom_cfg_saved or custom_cfg:
        ...
        if custom_cfg and custom_cfg not in lst:
...
def build_layout(search, serializing=False):
    ...
    for key, values in Matrix.settings.items():
        ...
        tag = dcc.Dropdown(
            id=f"list-settings-{sanitize_setting_key(key)}",
            options=options,
            **attr,
            clearable=False,
        )
    ...
    config = [
        html.B("Configuration:", id="config-title"),
        html.Br(),
        dcc.Input(
            id="custom-config",
    ...
    permalink_download = [
        html.P(
            [
                html.A("Permalink", href="", id="permalink"),
                " ",
    ...
    if not serializing:
        ...
        control_children += [
            html.I(["Saved on ", str(datetime.datetime.today()).rpartition(":")[0]])
        ]
        ...
        control_children += [
            html.P(["from ", html.A("this page", target="_blank", href=permalink), "."])
        ]
    ...
    if serializing:
        for idx, stats_name in enumerate(stats):
            ...
            graph_children += [
                dcc.Graph(id=table_stat.id_name, style={}, config=dict(showTips=False)),
        ...
        for idx, graph_id in enumerate(GRAPH_IDS):
            ...
            graph_children += [
                dcc.Graph(id=graph_id, style={"display": display}, config=dict(showTips=False)),
...
def build_callbacks(app):
    ...
    def download_graph():
        ...
        fname = (
            "__".join(TableStats.stats_by_name[stat_name].id_name for stat_name in query["stats"])
            if query.get("stats")
            else "nothing"
        )
    ...
    @app.callback(
        [
            Output("custom-config-saved", "children"),
            Output("custom-config-saved", "data-label"),
            Output("custom-config", "value"),
        ],
        [Input("config-title", "n_clicks")],
        [State("custom-config-saved", "data-label"), State("custom-config", "value")],
    )
    def save_config(*args):
        ...
        if value.startswith("_"):
            if value[1:] not in data:
                logging.warning(
                    f"tried to remove '{value[1:]}' but it's not in '{', '.join(data)}'"
                )
    ...
    @app.callback(
        [Output("settings-order", "children"), Output("settings-order", "data-order")],
        [Input(f"label_{sanitize_setting_key(key)}", "n_clicks") for key in Matrix.settings]
        + [Input("settings-order", "n_clicks")],
    ...
    def varname_click(*args):
        ...
        try:
            ...
            triggered_id = None  # nothing triggered the script (on multiapp load)
        ...
        if triggered_id == "settings-order.n_clicks":
            ...
            if setting_name in settings_order:
                settings_order.remove(setting_name)
            settings_order.insert(0, setting_name)

        try:
            settings_order.remove("stats")
        except ValueError:
            pass
    ...
    def display_hover_data(*args):
        ...
        try:
            ...
            return  # nothing triggered the script (on multiapp load)
        if triggered_id == ".":
            return
        ...
        variables = dict(zip(Matrix.settings.keys(), args[2 * NB_GRAPHS :], strict=False))
        ...
        if not meta:
            return "Error: no meta found for this graph ..."
        if "name" not in meta:
            return "Error: meta found for this graph has no name ..."

        obj = TableStats.stats_by_name[meta["name"]]
        return obj.do_hover(meta.get("value"), variables, figure, data, click_info)

    @app.callback(
        [Output("permalink", "href"), Output("download", "href")],
    ...
    def get_permalink_cb(*args):
        try:
            ...
            return dash.no_update, dash.no_update  # nothing triggered the script (on multiapp load)
    ...
    for _graph_idx, _graph_id in enumerate(GRAPH_IDS):
        ...
        def create_callback(graph_idx, graph_id):
            @app.callback(
                [Output(graph_id, "style"), Output(graph_id + "-txt", "style")],
            ...
            def graph_style(stats_values):
                ...
                if (
                    graph_idx != "graph-for-dl"
                    and (graph_idx + 1) > len(stats_values)
                    or not triggered_id
                    or not (stats_values and stats_values[0])
                ):
                    return (
                        {"display": "none"},
                        {"display": "none"},
                ...
                graph_style["height"] = (
                    f"{100 / (min(NB_GRAPHS, len(stats_values))) if stats_values else 100:.2f}vh"
                )
            ...
            @app.callback(
                [Output(graph_id, "figure"), Output(graph_id + "-txt", "children")],
                [
                    Input(f"list-settings-{sanitize_setting_key(key)}", "value")
                    for key in Matrix.settings
                ]
            ...
            def graph_figure(*_args):
                try:
                    ...
                    triggered_id = "<manually triggered>"
                ...
                *args, cfg_n_clicks, config, config_saved, config_init = _args
                ...
                cfg = Cfg()
                lst = (
                    (config_saved if config_saved else [])
                ...
                settings = dict(
                    zip(Matrix.settings.keys(), args[: len(Matrix.settings)], strict=False)
                )
                ...
                if graph_idx != "graph-for-dl" and (
                    not stats_values or (graph_idx + 1) > len(stats_values)
                ):
                    return dash.no_update, dash.no_update
                ...
                try:
                    plot_msg = table_stat.do_plot(
                        ordered_vars, settings, setting_lists, variables, cfg
                    )
                ...
                help_msg = [
                    ...
                    html.Ul([html.Li([html.I(k), " ⇒ ", cfg.d.get(k, "")]) for k in cfg.requests]),
                ]
