...
class _Report:
    ...
    def _children_element_to_html(self, elt):
        props = " ".join(
            [
                f"{k}='{getattr(elt, k)}'"
                for k in elt.available_properties
                if k not in ("children", "style") and hasattr(elt, k)
            ]
        )
        ...
        content = [
            f"<{elt._type.lower()}{' ' if props else ''}{props}{'/' if not elt.children else ''}>"
        ]
    ...
    def _graph_element_to_html(self, graph):
        ...
        from .web import IMAGE_HEIGHT, IMAGE_WIDTH
...
""")
        ...
        for i, tab in enumerate(tabs.children):
            ...
            content.append(
                f"<button class='tab-header{active_class}' onclick=\"showTab('{container_id}', '{tab_ids[i]}', this)\">{label}</button>"
            )
        content.append("</div>")
    ...
    def generate(self, content, report_index_f, include_header):
        ...
        if include_header:
            header = [
                ...
                "<p><a href='reports_index.html'>Back to the reports index.</a></p><hr>",
            ]
            html += header
        ...
        if report_index_f is not None:
            print(
                f"<li><a href='{dest}'> Report {self.index:02d}: {self.id_name.replace('_', ' ')}</a>",
                file=report_index_f,
            )
