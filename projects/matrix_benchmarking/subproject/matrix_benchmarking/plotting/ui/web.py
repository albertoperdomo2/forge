...
def construct_dispatcher():
    ...
    main_app.layout = html.Div([dcc.Location(id="url", refresh=False), html.Div(id="page-content")])

    @main_app.callback(
        Output("page-content", "children"), [Input("url", "pathname"), Input("url", "search")]
    )
...
def run():
    ...
    if generate:
        ...
        for graph, text in zip(content[0::2], content[1::2], strict=False):
    ...
    try:
        ...
        if e.errno == 98:
            logging.error("Dash server port already in use ...")
        else:
