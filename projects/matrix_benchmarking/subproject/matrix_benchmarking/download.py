...
def main(
    ...
    def download_an_entry(an_entry, workload_store):
        ...
        logging.info("Downloading an entry ...")
        ...
        def download(dl_mode):
            scrapper = scrapper_class(
                workload_store, source_url, base_dir, dest_dir, do_download, dl_mode
            )
            scrapper.scrape()
        ...
        def download_prefer_cache():
            if hasattr(workload_store, "load_cache"):
                ...
                for cache_file in dest_dir.glob("**/" + workload_store.CACHE_FILENAME):
                    try:
                        ...
                        if workload_store.load_cache(cache_file.parent):
                            logging.info(
                                f"Validation the cache of '{cache_file.parent}' succeeded."
                            )
                            successes += 1
                    except FileNotFoundError as e:
                        logging.warning(
                            f"Validation of the cache of '{cache_file.parent}' failed: {e} :/"
                        )
                        failed += 1
                ...
                logging.info("PREFER_CACHE downloading failed. Switching to IMPORTANT mode.")
