...
def _extract_metrics_from_prometheus(tsdb_path, process_metrics):
    ...
    subprocess.run(
        ["prometheus", "--help"], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT
    )
    ...
    try:
        ...
        prom_cmd = ["prometheus", "--storage.tsdb.path", str(tsdb_path), "--config.file", prom_cfg]

        prom_proc = subprocess.Popen(prom_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
        ...
        for i in range(RETRY_COUNT):
            ...
            try:
                prom_connect = prometheus_api_client.PrometheusConnect(
                    url=PROMETHEUS_URL,
                    disable_ssl=True,
                )
        ...
        if failed or prom_proc.returncode:
            logging.info(
                "<Promtheus stderr>\n%s\n</Promtheus stderr>",
                prom_proc.stderr.read().decode("utf8").strip(),
            )
...
def extract_metrics(prometheus_tgz, metrics, dirname):
    ...
    with open(dirname / "metrics.txt", "w") as f:
        for metric in metrics:
            metric_name, metric_query = (
                list(metric.items())[0] if isinstance(metric, dict) else (metric, metric)
            )
    ...
    def process_metrics(prom_connect):
        ...
        if not up_query:
            logging.error(
                f"No 'up' metric available in the database at '{prometheus_tgz}'. Cannot proceed :/"
            )
            return
        ...
        for metric_name, metric_query, metric_file in missing_metrics:
            if "(" in metric_query:
                try:
                    values = prom_connect.custom_query_range(
                        query=metric_query, step=step, start_time=start_date, end_time=end_date
                    )
                ...
                metric_values = metrics_values[metric_name] = prom_connect.custom_query(
                    query=f"{metric_query}[60y]"
                )
    ...
    logging.info(
        "Launching Prometheus instance to grab %s",
        ", ".join([name for name, query, file in missing_metrics]),
    )
