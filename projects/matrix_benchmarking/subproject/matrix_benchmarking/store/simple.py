...
def parse_old_settings(filename):
    ...
    with open(filename) as f:
        for line in f.readlines():
            ...
            if not found:
                logging.error(
                    f"Cannot parse setting from {filename}: invalid line (no '='): {line.strip()}"
                )
                continue
...
def _parse_directory(results_dir, dirname):
    ...
    def add_to_matrix(results, extra_settings=None):
        ...
        store.add_to_matrix(
            entry_import_settings, pathlib.Path(dirname), results, exit_code, _duplicated_directory
        )
...
def parse_lts_data(lts_results_dir=None):
    ...
    for _this_dir, directories, files in path:
        ...
        for filename in files:
            if filename == download_lts.LTS_ANCHOR_NAME:
                continue
...
def parse_data(results_dir=None):
    ...
    def has_settings(files):
        if "settings" in files:
            logging.debug("Found deprecated 'settings' file ...")
            return True  # deprecated
        if "settings.yml" in files:
            logging.warning("Found settings file with invalid extention 'settings.yml' file ...")
            return True
