...
def load_workload_store(kwargs):
    ...
    if workload_base_dir := kwargs.get("workload_base_dir"):
        logging.info(
            f"Adding workload_base_dir='{workload_base_dir}' to the Python module path list"
        )
...
def add_to_matrix(
    import_settings, location, results, exit_code, duplicate_handler, matrix=common.Matrix
):
    ...
    try:
        processed_settings = _rewrite_settings(dict(import_settings), results, is_lts)
    ...
    if processed_key in matrix.processed_map:
        ...
        processed_settings["run"] = (
            str(processed_settings.get("run")) + "_" + datetime.datetime.now().strftime("%H%M%S.%f")
        )
        processed_key = matrix.settings_to_key(processed_settings)
        return
