...
class Matrix:
    ...
    def run(
        ...
        logging.info(
            f"Ran {len(expe_ran)} {'matrices' if len(expe_ran) > 1 else 'matrix'}: {', '.join(expe_ran)}"
        )
        ...
        if tracker.dry:
            ...
            logging.info(
                f"- {tracker.expe_cnt.executed} {'has' if tracker.expe_cnt.executed == 1 else 'have'} been executed,"
            )
        logging.info(
            f"- {tracker.expe_cnt.recorded} {'was' if tracker.expe_cnt.recorded == 1 else 'were'} already recorded,"
        )
...
""",
    ...
    def do_run_matrix(self, tracker, all_settings_items, context, yaml_expe):
        for settings_items in itertools.product(*all_settings_items):
            ...
            if path_tpl is None:
                raise ValueError(
                    "<top-level>.--path-tpl or <top-level>.expe[<expe>].--path-tpl must be provided."
                )
            ...
            if key in common.Matrix.processed_map or key in common.Matrix.import_map:
                logging.info(
                    f"experiment {tracker.expe_cnt.current_idx}/{tracker.expe_cnt.total} already recorded, skipping."
                )
                location = (
                    common.Matrix.processed_map[key].location
            ...
            try:
                ...
                logging.error(
                    f"cannot apply the path template '{path_tpl}': key '{e.args[0]}' missing from {settings}"
                )
            ...
            next_id = (
                (int(list(parent_dir_matches[-1].name.split("__"))[0]) + 1)
    ...
    def execute_benchmark(self, settings, context, tracker):
        if not tracker.dry and not context.remote_mode:
            ...
            for test_file, _content in self.yaml_desc.get("test_files", {}).items():
                content = (
                    ...
                    else yaml.dump(_content, default_flow_style=False, sort_keys=False)
                )
        ...
        try:
            ...
            logging.error(
                f"cannot apply the script template '{context.script_tpl}': key '{e.args[0]}' missing from {settings}"
            )
            tracker.expe_cnt.errors += 1
            if context.stop_on_error:
                return False
