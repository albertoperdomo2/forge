...
def main(workload: str = "", workload_base_dir: str = "", file: str = "-"):
    """
...
def create_opensearch_mapping(json_schema):
    ...
    def process(path, entry, dest):
        ...
        if path != "#" and "title" in entry and "type" not in entry:
            entry["type"] = "object"
            entry["index"] = False
        ...
        for k, v in entry.items():
            if k == "$ref":
                ...
                "required",
                "pattern",
                "value",  # ignore, not supported
                "description",
                "enum",
                "items",
                "definitions",  # ignore, already processed
            ):
                continue  # ignore
