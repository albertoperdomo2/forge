...
class ScrapS3(BaseScapper):
    def download_file(self, filepath_rel, local_filename, depth, handler):
        ...
        s3_client.download_file(
            self.source_url.host, str(self.base_dir / filepath_rel).strip("/"), local_filename
        )
    ...
    def scrape(self, current_href=None, depth=0, test_found=False, handler=None):
        ...
        response = s3_client.list_objects_v2(
            Bucket=self.source_url.host, Prefix=str(current_dir).strip("/") + "/", Delimiter="/"
        )

        filenames = [pathlib.Path(entry["Key"]).name for entry in response.get("Contents", [])]
        dirnames = [
            pathlib.Path(entry["Prefix"]).name for entry in response.get("CommonPrefixes", [])
        ]
        ...
        for dirname in dirnames:
            ...
            if cache_found and self.download_only_cache:
                logging.info(
                    f"{' ' * depth}Directory: {new_href.relative_to(self.base_dir)}: SKIP (cache found)"
                )
                continue
