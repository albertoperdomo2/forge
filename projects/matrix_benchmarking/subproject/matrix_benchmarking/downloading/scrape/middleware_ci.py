...
class ScrapMiddlewareCiArtifacts(BaseHttpScapper):
    def scrape(self, current_href=None, depth=0, test_found=False):
        ...
        links = [
            svg.parent.next_sibling.find("a") for svg in s.find_all("svg", {"class": "icon-sm"})
        ]
        ...
        for link in links:
            ...
            svg_class = link.parent.previous_sibling.find("svg").attrs[
                "class"
            ]  # always exists, because of the way links/link is located
