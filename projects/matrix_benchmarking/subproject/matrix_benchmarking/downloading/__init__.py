...
class BaseScapper:
    def __init__(
        self, workload_store, source_url, base_dir, result_local_dir, do_download, download_mode
    ):
        ...
        self.download_only_cache = self.download_mode in (
            DownloadModes.PREFER_CACHE,
            DownloadModes.CACHE_ONLY,
        )
    ...
    def handle_file(self, filepath_rel, local_filename, depth, handler=None):
        ...
        important = (
            True
            if cache or mandatory
            else self.workload_store.is_important_file(result_filepath_rel)
        )

        only_important_files = self.download_mode in (
            DownloadModes.IMPORTANT,
            DownloadModes.PREFER_CACHE,
        )
