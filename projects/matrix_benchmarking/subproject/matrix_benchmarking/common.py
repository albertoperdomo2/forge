...
class MatrixEntry(types.SimpleNamespace):
    def __init__(
        ...
        matrix.import_map[import_key] = matrix.processed_map[processed_key] = self
    ...
    def get_name(self, variables) -> str:
        return ", ".join(
            [
                f"{key}={self.settings.__dict__[key]}"
...
class MatrixDefinition:
    ...
    def similar_records(
        ...
        ignore_keys,
        gathered=False,
        rewrite_settings=lambda x: x,
        ignore_lts_meta_keys=True,
    ):
    ...
    def all_records(
        self, settings=None, setting_lists=None, gathered=False
        ...
        for settings_values in sorted(
            itertools.product(*setting_lists), key=lambda x: x[0][0] if x else None
        ):
    ...
    def count_records(self, settings=None, setting_lists=None):
        ...
        return sum(
            [1 for _ in self.all_records(settings, setting_lists)]
        )  # don't use len(list(...)) with a generator, this form is more memory efficient

    def has_records(self, settings, setting_lists):
        try:
            _first_entry = next(
                self.all_records(settings, setting_lists)
            )  # raises an exception is the generator is empty
            return True
    ...
    def uniformize_settings_keys(self):
        ...
        for entry_key, entry in orig_processed_map.items():
            for settings_key in self.settings.keys():
                if settings_key in entry_key.settings:
                    continue
