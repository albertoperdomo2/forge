...
def get_rating_color(rating, improved=True):
    ...
    color = plotly.colors.sample_colorscale(color_scale, samplepoints=[rating])[0]
...
def generate_regression_analyse_report(
    regression_df, kpi_filter, comparison_keys, ignored_keys, sorting_keys
):
    ...
    report = []
    report.append(html.H1("Regression Analysis Summary"))
    ...
    for row_values in regression_df.values:
        row = dict(zip(regression_df, row_values, strict=False))

        for row_key, entry in zip(regression_df, row_values, strict=False):
            if is_nan(entry):
                continue
            if row_key == "ref":
                continue
            ...
            entry_settings = (
                entry.results.lts.metadata.settings
                if row_key == row["ref"]
    ...
    for row_values in regression_df.values:
        ...
        for kpi in kpis:
            ...
            if isinstance(ref_kpi.value, list):
                continue
            ...
            for comparison_key_value in sorted(row.keys()):
                ...
                entry_name = {
                    k: entry.results.metadata.settings.__dict__[k] for k in comparison_keys
                }
                ...
                if formatted_value := format_kpi_value(
                    kpi_entry, want_raw_value=False, fmt_kpi=ref_kpi
                ):
            ...
            regr_result = analyze.do_regression_analyze(
                ref_kpi.value, historical_values, lower_better=lower_better, kpi_unit=ref_kpi.unit
            )
            ...
            entry_regr_results[kpi.replace(kpis_common_prefix, "")] = OvervallResult(
                regr_result.rating,
            ...
            if not include_this_kpi_in_report:
                continue

            comparison_df = _generate_sorted_pd_table(comparison_data, comparison_keys).sort_values(
                by="ref"
            )
            ...
            if INCLUDE_REGRESSION_PLOT:
                entry_report.append(
                    _generate_comparison_plot(
                        comparison_df, comparison_keys, kpi, ref_kpi, kpis_common_prefix
                    )
                )

        if not include_this_entry_in_report:
            continue
    ...
    if no_history > 0:
        summary_html.append(
            html.P(
                f"No historical records for {no_history} entrie{'s' if no_history > 1 else ''} ..."
            )
        )
    ...
    kpi_names = (
        set(
            list(all_regr_results_data[0].keys())[
                max([metadata_column_count, len(variables) + 1]) :
            ]
        )
        if all_regr_results_data
        else set()
    )

    summary_html.append(
        _generate_results_overview(all_regr_results_data, variables, kpi_names, kpis_common_prefix)
    )
    ...
    if not_analyzed:
        summary += (
            f" {not_analyzed} entries couldn't be analyzed (check the logs for an explanation). "
        )
    if significant_performance_decrease:
        summary += (
            f" {significant_performance_increase} KPIs had a significant performance decrease. "
        )
    if significant_performance_increase:
        summary += (
            f" {significant_performance_increase} KPIs had a significant performance increase. "
        )
...
def generate_and_save_regression_analyse_report(
    ...
    report, failures, yaml_summary = generate_regression_analyse_report(
        regression_df, kpi_filter, comparison_key, ignored_keys, sorting_keys
    )
...
def _generate_entry_header(metadata_settings, metadata):
    ...
    lts_metadata_html = lts_metadata_df.style.hide(axis="index").set_table_styles(
        table_styles=STYLE_TABLE + STYLE_HIDE_COLUMN_TITLES
    )
    ...
    kpi_settings_html = kpi_settings_df.style.hide(axis="index").set_table_styles(
        table_styles=STYLE_TABLE + STYLE_HIDE_COLUMN_TITLES
    )
...
def _generate_sorted_pd_table(comparison_data, comparison_keys, warn=True):
    ...
    for comparison_key in comparison_keys:
        common_prefixes[comparison_key] = longestCommonPrefix(
            list(comparison_df[comparison_key].values)
        )
...
def _generate_comparison_table(comparison_df, kpi_unit):
    ...
    comparison_df_html = comparison_df.style.format(comparison_format).set_table_styles(
...
def _generate_evaluation_results(regr_result):
    ...
    regr_results_evaluation_fmt = dict(rating=rating_fmt)

    def regr_results_evaluation_style(row):
        fmt = []
        for key, value in zip(row.keys(), row.values, strict=False):
            if key in ("improved", "accepted"):
                color = (
                    (COLOR_IMPROVED if value else COLOR_DEGRADED)
    ...
    regr_df_html = (
        ...
        .style.format(regr_results_evaluation_fmt)
...
def _generate_lts_overview(all_lts_settings, lts_variables):
    ...
    def lts_config_overview_conditional_format(row):
        color = COLOR_OVERVIEW_VARIABLE if row["name"] in lts_variables else COLOR_OVERVIEW_FIX

        return [f"background: {color}"] * len(row)

    lts_config_overview_df = pd.DataFrame(lts_config_overview_data)
    lts_config_overview_df_html = (
        lts_config_overview_df.style.apply(lts_config_overview_conditional_format, axis=1)
        .hide(axis="index")
        .set_table_styles(table_styles=STYLE_TABLE)
    )
...
def _generate_configuration_overview(all_settings, variables):
    ...
    def config_overview_conditional_format(row):
        color = COLOR_OVERVIEW_VARIABLE if row["name"] in variables else COLOR_OVERVIEW_FIX

        return [f"background: {color}"] * len(row)

    config_overview_df = pd.DataFrame(config_overview_data)
    config_overview_df_html = (
        config_overview_df.style.apply(config_overview_conditional_format, axis=1)
        .hide(axis="index")
        .set_table_styles(table_styles=STYLE_TABLE)
    )
...
def _generate_results_overview(
    all_regr_results_data, variables, kpi_names, kpis_common_prefix, warn=True
):
    ...
    overview_fmt = dict.fromkeys(kpi_names, fmt)
    ...
    all_regr_results_df_html = (
        all_regr_results_df.style.apply(get_all_regr_results_conditional_format(variables), axis=1)
        .format(overview_fmt)
        .hide(axis="index")
...
def validate_regression_result(regr_result):
    ...
    if "inf" in str(regr_result.rating):
        raise ValueError("The regr_result rating shouldn't be infinite ...")

    if regr_result.rating < 0:
        raise ValueError(
            f"The regr_result rating ({regr_result.rating:.2f}) should not be negative"
        )

    range_0_1 = 0 <= regr_result.rating < 1
    if not ((regr_result.accepted and range_0_1) or (not regr_result.accepted and not range_0_1)):
        if not regr_result.improved:
            raise ValueError(
                f"The regr_result rating ({regr_result.rating:.2f}) should be between 0 and 1 only when the value is accepted ({regr_result.accepted})..."
            )


def _generate_details_table(regr_result):
    details_data = (
        [regr_result.details] if isinstance(regr_result.details, dict) else regr_result.details
    )
    regr_df_html = (
        pd.DataFrame(details_data)
        .style.apply(regr_result.details_conditional_fmt, axis=1)
...
def _generate_comparison_plot(
    _comparison_df, comparison_keys, kpi_name, ref_kpi, kpis_common_prefix
):
    data = []
    ref = None
...
def format_kpi_value(kpi, want_raw_value=True, fmt_kpi=None):
    ...
    if kpi.value is None:
        ...
        if kpi_is_list:
            return (
                ", ".join([fmt_kpi.format.format(v) for v in kpi_value])
                + f" {fmt_kpi.divisor_unit}"
            )
        else:
