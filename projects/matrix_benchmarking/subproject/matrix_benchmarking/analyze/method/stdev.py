...
def do_regression_analyze(current_value, historical_values, lower_better, kpi_unit):
    ...
    improved = (
        (below and lower_better) or (not below and not lower_better)
...
def get_std_dev_measurements(deviation, current_mean, previous_data):
    ...
    if deviation == 1:
        ...
        number_of_observation = len(
            [item for item in previous_data if item <= std_upper_bound and item >= std_lower_bound]
        )
        ...
        number_of_observation_below_the_mean = len(
            [
                item
                for item in previous_data
                if item <= std_lower_bound_1 and item >= std_lower_bound_2
            ]
        )
        number_of_observation_above_the_mean = len(
            [
                item
                for item in previous_data
                if item >= std_upper_bound_1 and item <= std_upper_bound_2
            ]
        )

        number_of_observation = (
            number_of_observation_above_the_mean + number_of_observation_below_the_mean
        )
    ...
    pct = 0 if len(previous_data) <= 1 else (number_of_observation / len(previous_data)) * 100
