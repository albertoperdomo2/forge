...
def _has_user_monitoring():
    ...
    try:
        monitoring_cm = kube.corev1.read_namespaced_config_map(
            namespace="openshift-monitoring", name="cluster-monitoring-config"
        )
...
def _get_prometheus_podinfo():
    ...
    pods = kube.corev1.list_namespaced_pod(
        namespace=podinfo.namespace, label_selector=PROM_POD_LABEL
    )
...
def do_query(handler, api_cmd, **data):
    ...
    resp = _exec_in_pod(
        handler.prom_podinfo.namespace,
        handler.prom_podinfo.podname,
        handler.prom_podinfo.container,
        curl_cmd,
    )
...
def get_handler():
    if not _has_user_monitoring():
        logging.error(
            """User monitoring not enabled. See https://docs.openshift.com/container-platform/4.10/monitoring/enabling-monitoring-for-user-defined-projects.html#enabling-monitoring-for-user-defined-projects_enabling-monitoring-for-user-defined-projects"""
        )
        raise SystemExit(1)
...
def restart_prometheus():
    ...
    logging.info("Prometheus Pod is running again.")
...
def query_current_ts(handler):
    try:
        metric = handler.prom_connect.get_current_metric_value(
            metric_name="cluster:memory_usage:ratio"
        )
...
def query_values(handler, metric, ts_start, ts_stop):
    minutes = math.ceil(
        (
            datetime.datetime.fromtimestamp(ts_stop) - datetime.datetime.fromtimestamp(ts_start)
        ).seconds
        / 60
    )
    return handler.prom_connect.custom_query(
        query=f"{metric}[{minutes}m]", params=dict(time=ts_stop)
    )
