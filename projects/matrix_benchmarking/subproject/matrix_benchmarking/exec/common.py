...
def is_connected():
    try:
        version_dict = kube.custom.get_cluster_custom_object(
...
def create_artifact_dir(benchmark_name):
    ...
    if sys.stdout.isatty():
        base_dir = pathlib.Path("/tmp") / (
            "matrix-benchmarking" + datetime.datetime.today().strftime("%Y%m%d")
        )
...
def save_system_artifacts():
    ...
    def save_nodes():
        ...
        try:
            del nodes_dict["metadata"]["managed_fields"]
        except KeyError:
            pass  # ignore

        try:
            del nodes_dict["status"]["images"]
        except KeyError:
            pass  # ignore
...
def finalize_prometheus(prom_data=None):
    ...
    def save_db_json():
        ...
        db_json = prom.dump_prometheus_db_json(
            prom_data.handler, prom_data.start_ts, prom_data.end_ts
        )
