...
def cleanup_previous_guidellm_resources_task(args, ctx):
    ...
    benchmark_name = ctx.config.benchmark["job_name"]
...
def create_guidellm_resources_task(args, ctx):
    ...
    llmd_runtime.apply_manifest(
        ctx.config.artifact_dir / "src" / "guidellm-pvc.yaml",
        llmd_runtime.render_guidellm_pvc(ctx.config),
    )
    llmd_runtime.apply_manifest(
        ctx.config.artifact_dir / "src" / "guidellm-job.yaml",
        llmd_runtime.render_guidellm_job(ctx.config, args.endpoint_url),
    )
    return f"GuideLLM benchmark {ctx.config.benchmark['job_name']} created"
...
def wait_guidellm_benchmark_task(args, ctx):
    ...
    llmd_runtime.wait_until(
        ...
        timeout_seconds=ctx.config.benchmark["timeout_seconds"],
        interval_seconds=10,
...
def capture_guidellm_state_task(args, ctx):
    ...
    capture_guidellm_state(ctx.config)
    return f"GuideLLM benchmark {ctx.config.benchmark['job_name']} state captured"
...
def copy_guidellm_results_task(args, ctx):
    ...
    copy_guidellm_results(ctx.config)
    return f"GuideLLM benchmark {ctx.config.benchmark['job_name']} results copied"
