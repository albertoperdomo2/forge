...
def test_single_flavor(flavor, flavor_index, total_flavors, namespace):
    ...
    with env.NextArtifactDir(f"flavor_{flavor}"):
        try:
            ...
            _, _, llmisvc_path = deploy_llm_inference_service(
                flavor, llmisvc_name, namespace, model_ref
            )
...
def test():
    ...
    if config.project.get_config("tests.llmd.inference_service.skip_deployment"):
        if len(flavors) != 1:
            raise ValueError(
                "tests.llmd.inference_service.skip_deployment is set "
    ...
    try:
        ...
        logging.warning(
            f"Failed to perform final cleanup of LLM inference service resources: {cleanup_e}"
        )
...
def _generate_test_metadata(failed, flavor):
    ...
    settings_data = {"llm-d": True, "flavor": flavor}
...
def generate_visualization(test_artifact_dir, test_failed):
    ...
    with env.NextArtifactDir("plots"):
        exc = run.run_and_catch(
            exc, visualize.generate_from_dir, test_artifact_dir, test_failed=test_failed
        )
        if not exc:
            logging.info(
                f"Test visualization has been generated into {env.ARTIFACT_DIR}/reports_index.html"
            )
        else:
    ...
    for part in parts[1:]:
        ...
        for prefix, (field, desc, format_example) in prefix_map.items():
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid {desc} specification: {part}. Expected format: {format_example}"
                )

        if not matched:
            raise ValueError(
                f"Unknown flavor component: {part}. Expected format: <base>-[tp<number>|dtp<number>]-[x<number>]-[ptp<number>]-[px<number>]"
            )
...
def apply_flavor_modifications(isvc_data, flavor):
    ...
    if base_flavor == "simple":
        ...
        # Unknown flavor
        raise ValueError(
            f"Unknown base flavor '{base_flavor}'. Supported flavors: simple, intelligentrouting"
        )
    ...
    if base_flavor == "pd":
        if prefill_replicas is not None:
            ...
            logging.info(
                f"Setting spec.prefill.replicas = {final_prefill_replicas} (p.x{prefill_replicas} * x{replicas if replicas is not None else 1})"
            )
...
def apply_kueue_configuration(isvc_data):
    ...
    if (
        "spec" in isvc_data
        and "router" in isvc_data["spec"]
        and "scheduler" in isvc_data["spec"]["router"]
    ):
        ...
        for annotation_key, annotation_value in kueue_annotations.items():
            ...
            logging.info(
                f"Added Kueue annotation to scheduler template: {full_annotation_key}={annotation_value}"
            )
    ...
    isvc_data["metadata"]["annotations"][f"{kueue_prefix}pod-group-total-count"] = str(
        pod_group_total_count
    )
    logging.info(
        f"Set pod-group-total-count: {pod_group_total_count} (1 scheduler + {replicas} replicas)"
    )
...
def apply_model_configuration(isvc_data):
    ...
    pvc_enabled = config.project.get_config(
        "prepare.pvc.enabled", True
...
def _apply_vllm_args_to_container_section(isvc_data, container_path, vllm_args, container_name):
    ...
    if new_args:
        ...
        logging.info(
            f"Set VLLM_ADDITIONAL_ARGS in {container_path}[{container_name}]: {combined_args}"
        )
...
def apply_max_model_len_configuration(isvc_data):
    ...
    _apply_vllm_args_to_container_section(
        isvc_data, "spec.template.containers", ["--max-model-len", f"{max_model_len}"], "main"
    )
    ...
    if "spec" in isvc_data and "prefill" in isvc_data["spec"]:
        ...
        _apply_vllm_args_to_container_section(
            isvc_data,
            "spec.prefill.template.containers",
...
def apply_image_pull_secrets_configuration(isvc_data):
    ...
    Args:
        isvc_data: The loaded YAML data structure
    """
    image_pull_secrets = config.project.get_config(
        "tests.llmd.inference_service.image_pull_secrets", None
    )
    ...
    if "router" in isvc_data["spec"] and "scheduler" in isvc_data["spec"]["router"]:
        isvc_data["spec"]["router"]["scheduler"]["template"]["imagePullSecrets"] = [
    ...
    if "prefill" in isvc_data["spec"]:
        isvc_data["spec"]["prefill"]["template"]["imagePullSecrets"] = [
            {"name": image_pull_secrets}
        ]
...
def apply_extra_properties(isvc_data):
    ...
    Args:
        isvc_data: The loaded YAML data structure
    """
    extra_properties = config.project.get_config(
        "tests.llmd.inference_service.extra_properties", {}
    )
...
def deploy_llm_inference_service(flavor, llmisvc_name, namespace, model_key):
    ...
    if config.project.get_config("tests.llmd.inference_service.skip_deployment"):
        logging.info(
            f"Skipping the deployment LLM inference service {llmisvc_name} in namespace {namespace}"
        )
    else:
...
def get_llm_inference_url(llmisvc_name, namespace, flavor):
    ...
    scheduler_result = run.run(
        f"oc get llmisvc/{llmisvc_name} -o jsonpath='{{.spec.router.scheduler}}' -n {namespace}",
        capture_stdout=True,
        check=False,
    )

    has_router_scheduler = (
        scheduler_result.returncode == 0 and scheduler_result.stdout.strip() != ""
    )

    # For services with intelligent routing, get the URL from status
    if has_router_scheduler:
        logging.info(
            "LLM inference service has router scheduler - looking up the gateway URL from status"
        )

        # Get the LLMInferenceService status addresses
        addresses_result = run.run(
            f"oc get llminferenceservice {llmisvc_name} -n {namespace} "
        ...
        if addresses_result.returncode != 0:
            raise RuntimeError(
                f"Failed to get LLMInferenceService addresses: {addresses_result.stderr}"
            )
        ...
        try:
            ...
            if not gateway_url:
                raise RuntimeError(
                    f"{gateway_name} URL not found in LLMInferenceService status addresses"
                )
    ...
    # For simple flavors, we need to append the HTTPS port from the service
        ...
        port_result = run.run(
            f"oc get svc {service_name} -n {namespace} "
...
def run_guidellm_benchmark(endpoint_url, llmisvc_name, namespace):
    ...
    def apply_rate_scaleup(value, rate):
        ...
        def evaluate_expression(match):
            ...
            try:
                ...
                logging.warning(
                    f"Failed to evaluate expression '{expression}' with rate={rate}: {e}"
                )
    ...
    for rate_value in rate_values:
        try:
            ...
            image_version = config.project.get_config(
                "tests.llmd.benchmarks.guidellm.image.version"
            )

            suffix = f"_rate{rate_value}" if len(rate_values) > 1 else None
...
def cleanup_llm_inference_resources():
    ...
    for i in range(6):  # Check up to 6 times with 10 second intervals
        result = run.run(
            f"oc get llminferenceservice -n {namespace} --no-headers", capture_stdout=True
        )
        ...
        if not result.stdout.strip():
            ...
            if i == 5:  # Last iteration
                logging.error(
                    f"Failed to clean up llminferenceservice resources after 60 seconds. {remaining_count} resources still exist:"
                )
                for service in remaining_services:
                    if service.strip():
                        logging.error(f"  - {service}")
                raise RuntimeError(
                    f"Cannot proceed with test - {remaining_count} llminferenceservice resources still exist in namespace {namespace}"
                )
...
def ensure_gpu_nodes_available():
    ...
    try:
        result = run.run(
            "oc get nodes -l nvidia.com/gpu.present=true --no-headers", capture_stdout=True
        )
        ...
        if not has_gpu_nodes:
            raise RuntimeError(
                "No GPU nodes found in the cluster. GPU nodes are required for LLM inference testing. Ensure prepare_gpu() was called."
            )
        else:
...
def test_llm_inference_simple(endpoint_url, llmisvc_name, namespace, model_name):
    ...
    try:
        ...
        logging.info(
            f"Simple LLM test successful. Completion: {test_payload['prompt']} ... {completion[:100]}"
        )
...
def conditional_scale_down():
    ...
    try:
        run.run_toolbox(
            "cluster", "set_scale", instance_type=node_instance_type, scale=0, mute_stdout=True
        )
