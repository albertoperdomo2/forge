...
PSAP_ODS_SECRET_PATH = pathlib.Path(
    os.environ.get("PSAP_ODS_SECRET_PATH", "/env/PSAP_ODS_SECRET_PATH/not_set")
)
...
def update_rhoai_pull_secret(secret_file_path):
    ...
    try:
        ...
        run.run(
            f'oc registry login --registry=quay.io/rhoai --auth-basic="{secret}" --to="{temp_file_path}"',
            log_command=False,
        )

        # Update cluster secret
        run.run(
            f'oc set data secret/pull-secret -n openshift-config --from-file=.dockerconfigjson="{temp_file_path}"',
            log_command=False,
        )
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            if not secret_json_valid:
                ...
                try:
                    ...
                    if not auth_data.get("auth") and not auth_data.get("username"):
                        logging.warning(
                            f"Registry {registry} missing authentication data, waiting..."
                        )
                        time.sleep(10)
                        continue
            ...
            if not machine_config_updated:
                ...
                result = run.run(
                    "oc get mcp -o jsonpath='{.items[*].status.conditions[?(@.type==\"Updated\")].status}'",
                    capture_stdout=True,
                )
            ...
            if test_image and not registry_access_tested:
                ...
                result = run.run(
                    "oc get nodes -l node-role.kubernetes.io/worker --no-headers -o custom-columns=NAME:.metadata.name | head -1",
                    capture_stdout=True,
                )
                if result.returncode == 0 and result.stdout.strip():
                    ...
                    test_result = run.run(
                        f"oc debug node/{test_node} -- chroot /host crictl pull {test_image}",
                        capture_stdout=True,
                        capture_stderr=True,
                        check=False,
                    )
                    if test_result.returncode != 0:
                        ...
                        logging.warning(
                            "Registry access test failed, but continuing with deployment"
                        )
                        time.sleep(15)
                        continue
                    ...
                    run.run(
                        f"oc debug node/{test_node} -- chroot /host crictl rmi {test_image}",
                        capture_stdout=True,
                        capture_stderr=True,
                        check=False,
                    )
            ...
            required_tests_complete = (
                secret_contains_registry and secret_json_valid and machine_config_updated
            )
...
def deploy_operator(operator_config):
    ...
    if deploy_cr is not None:
        args_dict["deploy_cr"] = True if deploy_cr is True else str(deploy_cr)
...
def prepare_grafana():
    ...
    for datasource_path in datasources or []:
        ...
        logging.info(
            f"Deploying Grafana datasource: {datasource_path} to namespace {grafana_namespace}"
        )
    ...
    if dashboards_dir_path:
        ...
        logging.info(
            f"Deploying {len(dashboard_files)} Grafana dashboard(s) from {dashboards_dir_path} to namespace {grafana_namespace}"
        )
...
def prepare_monitoring():
    ...
    run.run_toolbox(
        "cluster", "enable_userworkload_monitoring", namespaces=monitored_namespaces or []
    )
...
def scale_up():
    ...
    if node_instance_type and node_count:
        ...
        run.run_toolbox("cluster", "set_scale", instance_type=node_instance_type, scale=node_count)
...
"""
    ...
    run.run_toolbox(
        "rhods", "deploy_ods", catalog_image=rhoai_image, tag=rhoai_tag, channel=rhoai_channel
    )
...
def cleanup_cluster():
    ...
    if auto_scale_down and node_instance_type:
        ...
        run.run_toolbox("cluster", "set_scale", instance_type=node_instance_type, scale="0")
...
def download_models_to_pvc():
    ...
    if not pvc_enabled:
        logging.info(
            "PVC prefetch disabled (prepare.pvc.enabled: false) - skipping model downloads"
        )
        return
...
def download_single_model(model_key):
    ...
    try:
        ...
        check_result = run.run(
            f"oc get pvc -l {model_key}=yes -oname -n {namespace}", capture_stdout=True, check=False
        )

        expected_pvc = f"persistentvolumeclaim/{pvc_name}"
        if check_result.returncode == 0 and expected_pvc in check_result.stdout:
            logging.info(
                f"Model '{model_key}' already exists in PVC (found {expected_pvc} with label {model_key}=yes) - skipping download"
            )
            return

        logging.info(
            f"Model '{model_key}' not found in PVC - proceeding with download from {source}"
        )
        ...
        run.run_toolbox("llmd", "download_to_pvc", **download_args)
...
def preload_llm_model_image():
    ...
    try:
        ...
        if image_result.returncode != 0:
            raise RuntimeError(
                f"Failed to extract model URI from {yaml_file}: {image_result.stderr}"
            )
        ...
        if model_uri.startswith("oci://"):
            ...
            image_name = (
                run.run(f"cat {yaml_file} | yq .spec.model.name", capture_stdout=True)
                .stdout.strip()
                .strip('"')
            )
        ...
        csv_name_result = run.run(
            "oc get csv -n redhat-ods-operator -l operators.coreos.com/rhods-operator.redhat-ods-operator -oname",
            capture_stdout=True,
            check=False,
        )
        ...
        if csv_name_result.returncode != 0:
            ...
            csv_name = csv_name_result.stdout.strip().replace(
                "clusterserviceversion.operators.coreos.com/", ""
            )
            logging.info(f"Found RHODS operator CSV: {csv_name}")

            csv_result = run.run(
                f"oc get csv {csv_name} -n redhat-ods-operator -ojson | jq '.spec.relatedImages'",
                capture_stdout=True,
            )

            if csv_result.returncode != 0:
                logging.warning(
                    f"Failed to get RHODS operator related images (return code: {csv_result.returncode}): {csv_result.stderr}"
                )
                additional_images = []
            else:
                ...
                logging.info(
                    f"Found {len(additional_images)} additional images to preload out of {len(target_image_names)} targets"
                )
...
def preload_single_image(namespace, image, image_name=""):
    ...
    try:
        ...
        logging.error(
            f"Failed to preload image{f' {image_name}' if image_name else ''} {image}: {e}"
        )
        raise
