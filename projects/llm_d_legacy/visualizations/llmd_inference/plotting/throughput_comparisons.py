...
def _generate_ttft_percentiles_plot(args):
    ...
    dummy_df = pd.DataFrame(
        {
            "Test Configuration": configurations,
    ...
    for config_name in configurations:
        ...
        for perc_config in percentiles_config:
            ...
            if (
                column_name not in config_df.columns
                or config_df[column_name].isna().all()
                or config_df[column_name].isnull().all()
            ):
                continue
...
def _generate_throughput_plots(args):
    ...
    throughput_content.append(
        html.P("Token generation throughput scaling analysis using mean values.")
    )
    ...
    ttft_percentiles_content.append(
        html.P("Complete TTFT percentile distribution analysis (P10, P25, P50, P75, P90).")
    )
    ...
    itl_percentiles_content.append(
        html.P("Complete ITL percentile distribution analysis (P10, P25, P50, P75, P90).")
    )
    ...
    tabs = dcc.Tabs(
        ...
        children=[
            dcc.Tab(
                label="🚀 Throughput",
                value="throughput",
                children=[html.Div(throughput_content, style={"padding": "20px"})],
            ),
            dcc.Tab(
                label="⏱️ TTFT P50",
                value="ttft-p50",
                children=[html.Div(ttft_p50_content, style={"padding": "20px"})],
            ),
            dcc.Tab(
                label="📊 TTFT Percentiles",
                value="ttft-percentiles",
                children=[html.Div(ttft_percentiles_content, style={"padding": "20px"})],
            ),
            dcc.Tab(
                label="🔄 ITL P50",
                value="itl-p50",
                children=[html.Div(itl_p50_content, style={"padding": "20px"})],
            ),
            dcc.Tab(
                label="📊 ITL Percentiles",
                value="itl-percentiles",
                children=[html.Div(itl_percentiles_content, style={"padding": "20px"})],
            ),
        ],
    )
...
class BaselineComparisonsReport:
    ...
    def do_plot(self, *args):
        ...
        # Get simple flavors
        simple_flavors = [
            f for f in common.Matrix.settings.get("flavor", []) if f.startswith("simple")
        ]
        ...
        for with_simple in False, True:
            ...
            for load_shape in common.Matrix.settings.get("load_shape", []):
                ...
                def include_ir_flavor(v):
                    if v.startswith("pd-"):
                        return False
                    if not with_simple and v.startswith("simple"):
                        return False
                    if v.startswith("simple") and not v.endswith("tp2-x4"):
                        return False
    ...
    def do_plot(self, *args):
        ...
        for load_shape in common.Matrix.settings.get("load_shape", []):
            ...
            def include_pd_flavor(v):
                ...
                if not (v.startswith("pd-") or v.endswith("-tp4-x4")):
                    return False
                return True
