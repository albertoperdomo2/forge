...
def check_prometheus_data_availability(metric_type, report_name):
    ...
    for entry in entries:
        try:
            if (
                hasattr(entry.results, "metrics")
                and entry.results.metrics
                and entry.results.metrics.get(metric_type)
            ):
    ...
    if not has_metrics_data:
        logging.info(
            f"No {metric_type} Prometheus metrics data found - skipping {report_name} generation"
        )
        ...
        if metric_type == "uwm":
            error_elements = [
                ...
                html.P(
                    "To enable metrics collection, set tests.capture_prom_uwm: true in the configuration."
                ),
            ]
            ...
            error_elements = [
                ...
                html.P(
                    "This report requires cluster-level Prometheus metrics to be captured during the test run."
                ),
                html.P(
                    "To enable metrics collection, set tests.capture_prom: true in the configuration."
                ),
            ]
