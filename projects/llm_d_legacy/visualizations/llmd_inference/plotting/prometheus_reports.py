...
class PrometheusResourceUsageReport:
    ...
    def do_plot(self, *args):
        ...
        header.append(
            html.P("Comprehensive CPU and memory usage analysis for LLM-D inference workloads")
        )
        ...
        has_data, error_elements = check_prometheus_data_availability(
            "sutest", "resource usage report"
        )
        if not has_data:
...
class PrometheusGPUPerformanceReport:
    ...
    def do_plot(self, *args):
        ...
        header.append(
            html.P(
                "Comprehensive GPU utilization, memory, and throughput analysis using DCGM metrics"
            )
        )
        header.append(html.Br())
        ...
        header += report.Plot_and_Text(
            "Prom: Sutest GPU compute utilization (not 100% accurate)", args
        )
...
class PrometheusSystemHealthReport:
    ...
    def do_plot(self, *args):
        ...
        header.append(
            html.P("Kubernetes cluster health monitoring including API server and ETCD performance")
        )
        header.append(html.Br())
