...
class GuidellmThroughputScaling:
    ...
    def do_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        msg = []
        msg.append(
            f"Showing {total_strategies} strategies across {len(config_counts)} test configurations:"
        )
        msg.append(html.Br())
        ...
        msg.append(html.Br())
        msg.append(
            f"Best performing: {best_strategy['Strategy']} in {best_strategy['Test Configuration']} ({max_rate:.2f} req/s)"
        )
        msg.append(html.Br())
        msg.append(
            f"Performance range: {min_rate:.2f} - {max_rate:.2f} req/s ({performance_ratio:.1f}% spread)"
        )
        msg.append(html.Br())
        msg.append(
            "Note: Bubble size shows token throughput, color shows test configuration, shape shows strategy type"
        )
...
class GuidellmLatencyVsThroughput:
    ...
    def do_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        msg = []
        msg.append(
            f"Highest throughput: {best_throughput['Strategy']} in {best_throughput['Test Configuration']} ({best_throughput['Request Rate (req/s)']:.2f} req/s)"
        )
        msg.append(html.Br())
        msg.append(
            f"Lowest latency: {best_latency['Strategy']} in {best_latency['Test Configuration']} ({best_latency['Latency (ms)']:.1f} ms)"
        )
        msg.append(html.Br())
        msg.append(
            f"Most efficient: {most_efficient['Strategy']} in {most_efficient['Test Configuration']} ({most_efficient['Efficiency']:.2f} req/s per ms)"
        )
        msg.append(html.Br())
...
class GuidellmLatencyOverview:
    ...
    def do_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        latency_spread = (
            worst_latency["Request Latency (ms)"] - best_latency["Request Latency (ms)"]
        )

        # Configuration performance summary
        config_latency = (
            df.groupby("Test Configuration")["Request Latency (ms)"].mean().sort_values()
        )
        ...
        msg = []
        msg.append(
            f"Best request latency: {best_latency['Strategy']} in {best_latency['Test Configuration']} ({best_latency['Request Latency (ms)']:.1f} ms)"
        )
        msg.append(html.Br())
        msg.append(
            f"Worst request latency: {worst_latency['Strategy']} in {worst_latency['Test Configuration']} ({worst_latency['Request Latency (ms)']:.1f} ms)"
        )
        msg.append(html.Br())
        ...
        msg.append(html.Br())
        msg.append(
            f"Best configuration overall: {best_config} ({config_latency[best_config]:.1f} ms avg)"
        )
        msg.append(html.Br())
        msg.append(
            f"Best TTFT: {best_ttft['Strategy']} in {best_ttft['Test Configuration']} ({best_ttft['TTFT (ms)']:.1f} ms)"
        )
        msg.append(html.Br())
    ...
    def do_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        for entry in entries:
            ...
            for benchmark in entry.results.guidellm_benchmarks:
                ...
                total_tokens_mean = getattr(
                    benchmark,
                    "tokens_per_second_mean",
                    getattr(benchmark, "tokens_per_second", benchmark.tokens_per_second),
                )
                output_tokens_mean = getattr(
                    benchmark,
                    "output_tokens_per_second_mean",
                    getattr(
                        benchmark, "output_tokens_per_second", benchmark.output_tokens_per_second
                    ),
                )
                input_tokens_mean = getattr(
                    benchmark,
                    "input_tokens_per_second_mean",
                    getattr(
                        benchmark, "input_tokens_per_second", benchmark.input_tokens_per_second
                    ),
                )
        ...
        msg = []
        msg.append(
            f"Analyzing {total_strategies} strategies across {len(config_counts)} test configurations:"
        )
        msg.append(html.Br())
        for config, count in config_counts.items():
            msg.append(f"• {config}: {count} strategies")
        ...
        msg.append(
            f"Best token throughput: {best_tokens['Strategy']} in {best_tokens['Test Configuration']} ({max_tokens:.0f} tok/s at concurrency {best_tokens['Concurrency']:.0f})"
        )
        msg.append(html.Br())
        msg.append(
            f"Token throughput range: {min_tokens:.0f} - {max_tokens:.0f} tok/s ({token_improvement:.1f}% improvement)"
        )
        msg.append(html.Br())
        msg.append(
            f"Most efficient: {best_efficiency['Strategy']} in {best_efficiency['Test Configuration']} ({(best_efficiency['Tokens/s'] / best_efficiency['Concurrency']):.0f} tok/s per concurrency unit)"
        )
        msg.append(html.Br())
        msg.append(html.Br())
        ...
        if use_platform_markers:
            msg.append(
                "Note: Color shows test configuration, line style shows platform, lines connect points for each configuration. Values shown are mean for average performance analysis."
            )
        else:
            msg.append(
                "Note: Color shows test configuration, lines connect points for each configuration. Values shown are mean for average performance analysis."
            )
...
class GuidellmLatencyAnalysisBase:
    ...
    def do_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        color_map = {
            config_name: available_colors[i % len(available_colors)]
        ...
        fig.update_layout(title_text=title, showlegend=True, hovermode="closest", height=500)
        ...
        config_performance = (
            df.groupby("Test Configuration")
            .agg({p50_col: "mean", p95_col: "mean"})
            .sort_values(p50_col)
        )
        ...
        msg = []
        msg.append(
            f"🏆 Best {config['name']} P50: {best_p50['Strategy']} in {best_p50['Test Configuration']} ({best_p50[p50_col]:.1f} ms)"
        )
        msg.append(html.Br())
        msg.append(
            f"🏆 Best {config['name']} P95: {best_p95['Strategy']} in {best_p95['Test Configuration']} ({best_p95[p95_col]:.1f} ms)"
        )
        msg.append(html.Br())
        msg.append(f"📊 Average {config['name']} P50: {avg_p50:.1f} ms, P95: {avg_p95:.1f} ms")
        msg.append(html.Br())
        msg.append(
            f"📈 {config['name']} variability: P50 range {p50_range:.1f} ms, P95 range {p95_range:.1f} ms"
        )
        msg.append(html.Br())
        msg.append(html.Br())
        msg.append(
            f"🥇 Best configuration overall: {best_config_name} (P50: {config_performance.loc[best_config_name, p50_col]:.1f} ms, P95: {config_performance.loc[best_config_name, p95_col]:.1f} ms)"
        )
        msg.append(html.Br())
        msg.append(html.Br())
        msg.append(
            f"📝 Note: Left subplot shows P50 (median), right subplot shows P95. {config['note_text']}"
        )
        msg.append(html.Br())
...
class GuidellmTTFTAnalysis:
    ...
    def do_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        if "TTFT P50 (ms)" in df.columns and not df["TTFT P50 (ms)"].isna().all():
            ...
            msg = []
            msg.append(
                f"🏆 Best TTFT P50: {best_ttft['Strategy']} in {best_ttft['Test Configuration']} ({best_ttft['TTFT P50 (ms)']:.1f} ms)"
            )
            msg.append(html.Br())
            ...
            msg.append(html.Br())
            msg.append(
                "💡 Lower TTFT values indicate better responsiveness. Look for configurations with consistently low and stable TTFT across concurrency levels."
            )
        else:
...
class GuidellmITLAnalysis:
    ...
    def do_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        if "ITL P50 (ms)" in df.columns and not df["ITL P50 (ms)"].isna().all():
            ...
            msg = []
            msg.append(
                f"🏆 Best ITL P50: {best_itl['Strategy']} in {best_itl['Test Configuration']} ({best_itl['ITL P50 (ms)']:.1f} ms)"
            )
            msg.append(html.Br())
            ...
            msg.append(html.Br())
            msg.append(
                "💡 Lower ITL values indicate smoother token streaming. Look for configurations with consistently low ITL for better user experience."
            )
        else:
    ...
    def do_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        for entry in entries:
            ...
            for benchmark in entry.results.guidellm_benchmarks:
                ...
                total_tokens_p50 = getattr(
                    benchmark, "tokens_per_second_median", benchmark.tokens_per_second
                )
                output_tokens_p50 = getattr(
                    benchmark, "output_tokens_per_second_median", benchmark.output_tokens_per_second
                )
                input_tokens_p50 = getattr(
                    benchmark, "input_tokens_per_second_median", benchmark.input_tokens_per_second
                )
        ...
        fig_stacked = px.bar(
            df_stacked, x="Strategy", y="Tokens/s", color="Token Type", title=breakdown_title
        )
        ...
        # Configuration performance summary
        config_tokens = (
            df.groupby("Test Configuration")["Total Tokens/s"].mean().sort_values(ascending=False)
        )
        best_config = config_tokens.index[0]

        msg = []
        msg.append(
            f"Highest token throughput: {best_strategy['Strategy']} in {best_strategy['Test Configuration']} ({max_tokens:.0f} tok/s)"
        )
        msg.append(html.Br())
        ...
        msg.append(html.Br())
        msg.append(
            f"Best configuration overall: {best_config} ({config_tokens[best_config]:.0f} tok/s avg)"
        )
        msg.append(html.Br())
        msg.append(html.Br())
        msg.append(
            "Note: All token throughput values shown are mean values for average performance analysis."
        )
    ...
    def do_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        for perc in percentiles_to_analyze:
            if perc in best_performers:
                ...
                msg.append(
                    f"• <b>Best {perc}</b>: {best['strategy']} in {best['config']} ({best['value']:.0f} tok/s at {best['concurrency']:.0f} concurrency)"
                )
                msg.append(html.Br())
        ...
        msg.append(html.Br())
        msg.append(
            "💡 <b>Analysis</b>: Smaller spreads between percentiles indicate more consistent performance. Compare mean vs P50 to understand distribution skew. Focus on configurations with high P50 and small P90—P10 gaps."
        )
