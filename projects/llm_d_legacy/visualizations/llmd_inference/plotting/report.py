...
class GuidellmResultsTable:
    ...
    def do_plot(self, *args):
            ...
            if hasattr(artifacts_basedir, "artifacts_basedir"):
                ...
                artifact_links.append(
                    html.Li(
                        [
                            html.A(
                                f"🗂️ {entry_name} - Test artifacts directory",
                                href=str(artifacts_basedir),
                                target="_blank",
                            )
                        ]
                    )
                ...
                if (
                    hasattr(entry.results, "guidellm_benchmarks")
                    and entry.results.guidellm_benchmarks
                ):
                    guidellm_dir = None
                    log_file = None
                    ...
                    for item in artifacts_basedir.iterdir():
                        if item.is_dir() and "__llmd__run_guidellm_benchmark" in item.name:
                            ...
                            potential_log = (
                                guidellm_dir / "artifacts" / "guidellm_benchmark_job.logs"
                            )
                    ...
                    if guidellm_dir:
                        guidellm_sublinks = [
                            html.Li(
                                html.A(
                                    "📊 GuideLLM benchmark directory",
                                    href=str(guidellm_dir),
                                    target="_blank",
                                )
                            )
                        ]
                        ...
                        artifact_links.append(html.Li(["GuideLLM:", html.Ul(guidellm_sublinks)]))
            else:
                # Get unique name for this entry (includes flavor info)
                entry_name = entry.get_name(variables) if variables else f"Test {i + 1}"
        ...
        best_throughput_idx = max(
            range(len(all_benchmarks)), key=lambda i: all_benchmarks[i].request_rate
        )
        best_latency_idx = min(
            range(len(all_benchmarks)), key=lambda i: all_benchmarks[i].ttft_median
        )
        best_tokens_idx = max(
            range(len(all_benchmarks)), key=lambda i: all_benchmarks[i].tokens_per_second
        )
        ...
        for i, benchmark in enumerate(all_benchmarks):
            ...
            if config not in configurations:
                configurations[config] = {"throughput": [], "latency": [], "tokens": [], "count": 0}
    ...
    def _create_html_table(self, headers, rows):
        ...
        cell_style = {"padding": "8px", "text-align": "left", "border": "1px solid #ddd"}
...
class GuidellmPerformanceAnalysisReport:
    ...
    def do_plot(self, *args):
        ...
        header.append(
            html.P(
                "Comprehensive performance analysis including token throughput scaling and TTFT latency patterns across different test configurations"
            )
        )
        header.append(html.Br())
        ...
        if not has_guidellm_data:
            ...
            header.append(
                html.P(
                    "This report requires GuideLLM benchmark results to analyze token throughput scaling."
                )
            )
            header.append(
                html.P(
                    "To enable benchmarks, set tests.llmd.benchmarks.guidellm.enabled: true in the configuration."
                )
            )
            return None, header
        ...
        header.append(
            html.P("Analysis of how token generation throughput scales with concurrency levels")
        )
        ...
        header.append(
            html.P(
                "Latency analysis showing response time percentiles across different concurrency levels"
            )
        )
        ...
        header.append(
            html.P(
                "Token generation speed analysis showing how quickly individual tokens are produced"
            )
        )
        ...
        header.append(
            html.P("Streaming responsiveness analysis showing the delay between consecutive tokens")
        )
        ...
        header.append(
            html.P(
                "Complete request duration analysis from request initiation to final response completion"
            )
        )
        ...
        if all_benchmarks:
            ...
            best_efficiency_idx = max(
                ...
                key=lambda i: (
                    all_benchmarks[i].tokens_per_second
                    / max(all_benchmarks[i].request_concurrency, 1)
                ),
            )
            ...
            sorted_configs = sorted(
                configurations.items(), key=lambda x: x[1]["max_tokens"], reverse=True
            )
            ...
            if len(sorted_configs) > 1:
                ...
                for i, (config, metrics) in enumerate(sorted_configs):
                    ...
                    header.append(
                        html.P(
                            f"{rank_emoji} {config}: {metrics['max_tokens']:.0f} tok/s (optimal concurrency: {metrics['optimal_concurrency']:.0f}, {metrics['strategies']} strategies)"
                        )
                    )
            ...
            header.append(
                html.P(
                    "💡 Use this analysis to identify optimal concurrency settings for maximum token generation throughput in your specific deployment configuration."
                )
            )
