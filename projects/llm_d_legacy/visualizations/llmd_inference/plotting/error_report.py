...
def analyze_llmisvc_deployment(llmisvc_config):
    ...
    prefill_containers = (
        safe_get(isvc_data, "spec.prefill.template.containers")
        if prefill_replicas != "missing"
        else "missing"
    )
    prefill_vllm_args = (
        get_vllm_args(prefill_containers) if prefill_replicas != "missing" else "missing"
    )
    prefill_resources = (
        safe_get(isvc_data, "spec.prefill.template.containers[0].resources")
        if prefill_replicas != "missing"
        else "missing"
    )
...
class LlmdInferenceErrorReport:
    ...
    def do_plot(self, *args):
            ...
            header.append(
                html.P(
                    [
                        "✅ Successful tests: ",
                        html.Span(
                            str(successful_tests), style={"color": "green", "font-weight": "bold"}
                        ),
                ...
                [
                    html.Li(
                        [
                            "🎯 GuideLLM benchmarks: ",
                            html.Span(
                                f"{guidellm_available}/{total_tests}", style={"font-weight": "bold"}
                            ),
                    ...
                    html.Li(
                        [
                            ...
                            html.Span(
                                f"{prometheus_available}/{total_tests}",
                    ...
                    html.Li(
                        [
                            ...
                            html.Span(
                                f"{llmisvc_available}/{total_tests}", style={"font-weight": "bold"}
                            ),
        ...
        for i, entry in enumerate(entries):
            ...
            header.append(html.P(["Settings: ", html.Span(labels_list)]))
                    ...
                    if guidellm_dir:
                        artifact_links.append(
                            html.Li(
                                [
                                    html.A(
                                        f"📊 GuideLLM benchmark results ({strategy_count} strategies)",
                        ...
                        log_file = (
                            guidellm_dir
                            / "000__llmd__run_guidellm_benchmark"
                            / "artifacts"
                            ...
                            artifact_links.append(
                                html.Li(
                                    [
                                        "  └─ ",
                                        html.A(
                                            "benchmark logs", href=str(log_file), target="_blank"
                                        ),
                                    ]
                                )
                            )
        ...
        header.append(
            html.P(
                "Quick access to PROW artifacts for all test entries organized by platform → model → load_shape:"
            )
        )
        ...
        for i, entry in enumerate(entries):
            ...
            hierarchy[platform_with_version][model][load_shape].append(
                {"settings_str": settings_str, "prow_url": prow_url}
            )
                ...
                for load_shape in sorted(hierarchy[platform_version][model].keys()):
                    ...
                    for entry_data in hierarchy[platform_version][model][load_shape]:
                        if entry_data["prow_url"]:
                            list_item_content = [
                                html.A(
                                    entry_data["settings_str"],
                                    href=entry_data["prow_url"],
                                    target="_blank",
                            ...
                            list_item_content = [
                                f"{entry_data['settings_str']} (No PROW artifacts available)"
                            ]
