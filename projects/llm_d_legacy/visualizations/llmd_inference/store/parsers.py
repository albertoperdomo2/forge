...
artifact_dirnames.PROMETHEUS_DUMP_DIR = (
    "*__cluster__dump_prometheus_dbs/*__cluster__dump_prometheus_db"
)
artifact_dirnames.PROMETHEUS_UWM_DUMP_DIR = (
    "*__cluster__dump_prometheus_dbs/*__cluster__dump_prometheus_db_uwm"
)
...
def parse_once(results, dirname):
    ...
    results.from_env = helpers_store_parsers.parse_env(
        dirname, None, artifact_paths.LLMISVC_CAPTURE_DIR
    )
    ...
    if guidellm_directories:
        for guidellm_dir in guidellm_directories:
            ...
            if json_file_path.exists():
                benchmarks = parse_guidellm_benchmark_json(
                    dirname, json_file_path.relative_to(dirname)
                )
                results.guidellm_benchmarks.extend(benchmarks)
                logging.info(
                    f"Parsed {len(benchmarks)} guidellm benchmarks from JSON: {json_file_path}"
                )

                # Parse configuration from the first JSON file found
                if results.guidellm_configuration is None:
                    results.guidellm_configuration = _parse_guidellm_config(
                        dirname, json_file_path.relative_to(dirname)
                    )

            elif log_file_path.exists():
                raise RuntimeError("Don't want to use log-file parsing (hardcoded)")
                benchmarks = parse_guidellm_benchmark_log(
                    dirname, log_file_path.relative_to(dirname)
                )
                results.guidellm_benchmarks.extend(benchmarks)
                logging.info(
                    f"Parsed {len(benchmarks)} guidellm benchmarks from log: {log_file_path}"
                )
            else:
...
def parse_guidellm_benchmark_log(dirname, log_file_path: pathlib.Path) -> list[GuidellmBenchmark]:
    ...
    if "Run Summary Info" not in content:
        logging.warning(
            f"GuideLLM log appears to be incomplete or malformed - no 'Run Summary Info' section found in {log_file_path}"
        )
        return []
    ...
    latency_match = re.search(
        r"ℹ Request Latency Statistics.*?\n(.*?)(?=ℹ Server Throughput Statistics|\Z)",
        content,
        re.DOTALL,
    )
    ...
    throughput_match = re.search(
        r"ℹ Server Throughput Statistics.*?\n(.*?)(?=\n\n|\Z)", content, re.DOTALL
    )
    ...
    for i, summary_row in enumerate(summary_rows):
        ...
        try:
            ...
            throughput_parts = [
                part.strip() for part in throughput_rows[i].split("|") if part.strip()
            ]
            ...
            logging.info(
                f"Parsed Guidellm benchmark: {strategy}, rate={request_rate:.2f} req/s, concurrency={concurrency_mean:.1f}, tokens/s={total_tokens_per_second:.1f}"
            )

            # Highlight unexpected throughput values
            if total_tokens_per_second > 1000 or total_tokens_per_second < 1:
                logging.warning(
                    f"Unusual tokens/second value: {total_tokens_per_second:.1f} for strategy {strategy}"
                )

            # Debug logging for troubleshooting
            logging.debug(f"  Raw parsed values for {strategy}:")
            logging.debug(
                f"    Latency: TTFT={ttft_median:.3f}ms, TPOT={tpot_median:.3f}ms, ITL={itl_median:.3f}ms"
            )
            logging.debug(
                f"    Request Latency: median={request_latency_median:.3f}ms, p95={request_latency_p95:.3f}ms"
            )
            logging.debug(
                f"    Throughput: input={input_tokens_per_second:.1f}, output={output_tokens_per_second:.1f}, total={total_tokens_per_second:.1f}"
            )
...
def parse_guidellm_benchmark_json(dirname, json_file_path: pathlib.Path) -> list[GuidellmBenchmark]:
    ...
    try:
        ...
        logging.debug(
            f"Parsing GuideLLM JSON file: {json_file_path} (found {len(json_data.get('benchmarks', []))} benchmarks)"
        )
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        logging.warning(
                            f"Could not find concurrency 'streams' for benchmark. Using default value 1.0"
                        )
                        concurrency = 1.0
                        concurrency_source = "default"
                ...
                def get_metric_value(
                    metric_name, stat_type="median", default=0.0, fail_if_missing=False
                ):
                    ...
                    if stat_type in ["p95", "p90", "p75", "p50", "p25", "p10"]:
                        ...
                        if stat_type not in percentiles:
                            if fail_if_missing:
                                raise KeyError(
                                    f"Percentile {stat_type} not found in {metric_name} percentiles: {list(percentiles.keys())}"
                                )
                            return default
                        ...
                        if stat_type not in metric_data:
                            if fail_if_missing:
                                raise KeyError(
                                    f"Stat type {stat_type} not found in {metric_name} successful data: {list(metric_data.keys())}"
                                )
                            return default
                ...
                output_tokens_per_second_p10 = get_metric_value(
                    "output_tokens_per_second", "p10", fail_if_missing=True
                )
                output_tokens_per_second_p25 = get_metric_value(
                    "output_tokens_per_second", "p25", fail_if_missing=True
                )
                output_tokens_per_second_p50 = get_metric_value(
                    "output_tokens_per_second", "p50", fail_if_missing=True
                )
                output_tokens_per_second_p75 = get_metric_value(
                    "output_tokens_per_second", "p75", fail_if_missing=True
                )
                output_tokens_per_second_p90 = get_metric_value(
                    "output_tokens_per_second", "p90", fail_if_missing=True
                )
                ...
                benchmarks.append(benchmark)
                logging.info(
                    f"Parsed JSON benchmark: {strategy}, rate={request_rate:.2f} req/s, concurrency={concurrency:.1f} (from {concurrency_source}), tokens/s={total_tokens_per_second:.1f}"
                )

                # Debug logging
                logging.debug(f"  JSON parsed values for {strategy}:")
                logging.debug(
                    f"    Concurrency: {concurrency:.1f} (extracted from {concurrency_source})"
                )
                logging.debug(
                    f"    Latency: TTFT={ttft_median * 1000:.3f}ms, TPOT={tpot_median * 1000:.3f}ms, ITL={itl_median * 1000:.3f}ms"
                )
                logging.debug(
                    f"    Request Latency: median={request_latency_median * 1000:.3f}ms, p95={request_latency_p95 * 1000:.3f}ms"
                )
                logging.debug(
                    f"    Throughput: input={input_tokens_per_second:.1f}, output={output_tokens_per_second:.1f}, total={total_tokens_per_second:.1f}"
                )
...
def _parse_llmisvc_config(dirname):
    ...
    llmisvc_config_path = (
        artifact_paths.LLMISVC_CAPTURE_DIR / "artifacts" / "llminferenceservice.json"
    )
...
def _extract_metrics(dirname):
    ...
    if artifact_paths.PROMETHEUS_DUMP_DIR is not None:
        db_files["sutest"] = (
            str(artifact_paths.PROMETHEUS_DUMP_DIR / "prometheus.t*"),
            get_llmd_main_metrics(),
        )

    # Add UWM Prometheus metrics (application-level: vllm, etc.)
    if artifact_paths.PROMETHEUS_UWM_DUMP_DIR is not None:
        db_files["uwm"] = (
            str(artifact_paths.PROMETHEUS_UWM_DUMP_DIR / "prometheus.t*"),
            get_llmd_uwm_metrics(),
        )
